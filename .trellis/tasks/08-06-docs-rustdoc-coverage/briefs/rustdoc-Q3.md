...
Crate(s): `hya-store`. **0 undocumented public items.** Do not touch any other
crate -- other writers are working in parallel.

The crate-level `//!` is substantive; leave it unless it is wrong.

This crate is at 0% coverage. Document `SessionStore` first (it is the entire entry point), then `StoreError`, `SessionInfo`, `LedgerEntry`, then the admission state machine (`AdmissionState`, `AdmissionRecord`, `AdmissionClaimOutcome`, `AdmissionTerminal`), then `BundleRegistry` and `MAX_ADMISSION_INTENT_BYTES`.

The admission types are SAFETY-CRITICAL: they bound spawn budgets. docs/architecture/admission-and-governor.md documents this same machine -- read it and stay consistent with it.
