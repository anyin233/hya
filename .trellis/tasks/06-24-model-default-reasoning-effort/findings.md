...
- `ReasoningEffort` lives in `crates/yaca-provider/src/lib.rs` and orders efforts as `Off < Minimal < Low < Medium < High < XHigh < Max`. It already parses `off|none|minimal|low|medium|med|high|xhigh|max` and encodes provider labels/budgets.
- Provider support is exposed by `ProviderKind::reasoning_variants` in `crates/yaca-provider/src/http.rs`:
...
- Native TUI model selection receives `Vec<ModelEntry>` from `yaca_app::config`; `ModelEntry` currently stores only `{ id, provider }`, so the TUI cannot render model-specific `/think` variants without extending this type or deriving a second catalog input.
...
- `cargo test --workspace` currently fails unrelated `yaca-server` skill-order assertions in this environment because global `brainstorming` appears before test-local skills. Treat this as pre-existing unless the current work touches those tests.
