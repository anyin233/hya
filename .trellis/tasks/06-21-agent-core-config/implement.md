...
1. [ ] Capture baseline: `cargo test --workspace` green; `hya exec "hello"` (offline) transcript saved to `.omo/evidence/baseline.txt` (or `/tmp`). Re-confirm live `AgentSpec` fields (`reasoning`?).
2. [ ] Add `regex = "1"` to workspace `Cargo.toml` deps; add `serde_norway = { workspace = true }` + `regex = { workspace = true }` to `crates/hya-core/Cargo.toml`. Re-export `hya_tool::permission::glob_match` from `hya-tool/src/lib.rs`.
3. [ ] Validate: `cargo build --workspace`. **Rollback**: revert Cargo edits. Risky: `Cargo.lock`.

## Wave 1 — `ToolAllowlist` in hya-tool  [AC2 foundation]
4. [ ] RED `crates/hya-tool/tests/tool.rs`: `permits` cases (All admits any incl `ask_user`+`task`; Only excludes unlisted; Only admits listed). `schemas_for` filters+sorts — assert all **10** builtins (read/write/edit/ls/glob/find/grep/shell/ask_user/task); read-only `Only({read,ls,glob,find,grep})` advertises 5; `require_allowed` rejects disallowed, errors unknown.
5. [ ] GREEN: add `ToolAllowlist` enum + `ToolRegistry::schemas_for` + `require_allowed` ([tool.rs](../../../crates/hya-tool/src/tool.rs)). Export from `hya-tool/src/lib.rs`.
6. [ ] Gate. **Rollback**: revert hya-tool. Risky: `tool.rs`, `lib.rs`.
...
9. [ ] Gate `cargo test -p hya-core agent::config`. **Rollback**: rm `agent/`, revert `lib.rs`. Risky: `lib.rs`.
...
13. [ ] Gate `cargo test -p hya-core agent::loader`. **Rollback**: revert loader+builtin. Risky: builtin `build.md` must reproduce current persona byte-for-byte.
...
15. [ ] GREEN: move `SkillCatalog`/discovery into `agent/skills.rs` (CLI imports back); add related-skills section to `build_system_prompt` (extra `skills_section: Option<String>` param) ([prompt.rs:8](../../../crates/hya-core/src/prompt.rs)).
16. [ ] Gate `cargo test -p hya-core agent::skills` + `cargo build -p hya-cli`. Risky: cross-crate move (audit `use crate::skills` in hya-cli).
...
18. [ ] GREEN: `InjectionRule`/`Selector`/`Trigger`, `InjectionLibrary::load`, `compose_static`, `keyword_matches`, `model_family`, marker `<hya-injection name="X">` build/scan helpers.
19. [ ] Gate `cargo test -p hya-core agent::injection`.
...
21. [ ] RED `crates/hya-core/tests/turn_loop.rs` (use `FakeProvider::scripted_turns` [fake.rs](../../../crates/hya-provider/src/fake.rs) + `RecordingProvider`): agent `[read,ls]` advertises only read,ls (not write/edit/shell/ask_user); forced disallowed `write` call → `Event::ToolError "not allowed"`; advertise/execute agree (proptest).
22. [ ] GREEN: `request_from_messages` → `schemas_for(&agent.allowed_tools)` ([engine.rs:474](../../../crates/hya-core/src/engine.rs)); dispatch → `require_allowed` ([engine.rs:325-358](../../../crates/hya-core/src/engine.rs)).
23. [ ] RED injection integration (AC4): (a) keyword `ulw` → on a user prompt matching `(?i)(ultrawork|ulw)`, `run_turn` emits a logged `<hya-injection name="ulw">` System message; non-matching prompt → none; sibling agent not selected → none. (b) `once:true` → fires once across two matching turns (marker dedup). (c) compaction-correctness: after `compact_with` summarizes the turn, the marker remains in `read_projection` ⇒ NO double-fire — assert no re-injection; do NOT assert the directive is re-shown (post-compact recovery out of scope, D5). [always/session-start coverage is in step 12b.]
...
27a. [ ] GREEN: author builtin preset files (quick/deep/ultrabrain/writing `extends: build` + tier models) so model + extends assertions pass. Gate: `cargo test -p hya-core --test agent_preset_routing presets_resolve preset_extends`.
27b. [ ] GREEN: `MemberSpec → { id, agent: AgentName, directive }` ([subagent.rs:32](../../../crates/hya-core/src/subagent.rs)); `run_member(engine, lead, catalog, spec, cancel)` resolves `catalog.require(spec.agent)` → AgentCore (unknown ⇒ `MemberStatus::Failed`); `run_team(engine, lead, catalog, specs, cancel)`. `cargo build -p hya-core`.
27c. [ ] GREEN: `spawn_team_supervisor` ([main.rs:199-238](../../../crates/hya-cli/src/main.rs)) currently IGNORES `subagent_type` and clones `base` into every member — change it to hold `Arc<AgentCatalog>` and map each `SpawnMember.subagent_type` → `MemberSpec{ agent: AgentName::new(subagent_type) }`, passing the catalog into `run_team` (NEW behavior: members get their preset model; unknown ⇒ `Failed` in `run_member`). `cargo build -p hya-cli`.
27d. [ ] GREEN: delete `crates/hya-core/src/category.rs`; remove `pub mod category` + re-exports (`CategoryEntry/CategoryRegistry/ResolvedCategory/build_member_agent/inject_skills`, [lib.rs:18](../../../crates/hya-core/src/lib.rs)). `cargo build --workspace` (catches stragglers).
27e. [ ] Verify `TaskTool` `subagent_type` enum `{quick,deep,ultrabrain,writing}` ([tool.rs:581](../../../crates/hya-tool/src/tool.rs)) still resolves as agent names (no change unless a preset is renamed).
28. [ ] Gate `cargo test -p hya-core --test agent_preset_routing` + `cargo test --workspace`.

## Wave 8 — CLI / server / TUI wiring  [AC1 live]
29. [ ] GREEN: replace `agent_with_model(model)` ([main.rs:156](../../../crates/hya-cli/src/main.rs)) with `AgentCatalog::load(opts)?.require("build")?.clone()` + model override last (preserve `--model`/`HYA_MODEL`); build catalog once; pass into `build_session_engine`/team sites.
30. [ ] GREEN: `AppState.agent: Arc<AgentCore>` ([hya-server/src/lib.rs](../../../crates/hya-server/src/lib.rs)); TUI `/model` re-resolves model-dependent injections ([tui.rs](../../../crates/hya-cli/src/tui.rs)).
...
33. [ ] Manual QA (tmux/Bash, capture evidence to `.omo/evidence/`):
    - **AC1**: `hya exec "hello"` no-config → builtin persona transcript == baseline. Drop `.hya/agents/build.md` with a distinct persona → `hya exec --json "hi"` shows it; remove file → baseline restored.
    - **AC2**: define `.hya/agents/reader.md` (`allowed_tools:[read,ls,glob,find,grep]`); `hya exec` with reader → advertised tools exclude write/edit/shell/ask_user/task (assert via `--json`/provider trace); forced disallowed call rejected (Wave 6 test is the execute-side real surface).
    - **AC3** (Round-1 D5): `.hya/agents/reader.md` with `skills:[<one discovered skill>]` → `hya exec --json` shows that skill in reader's related-skills section; `build` (`skills: all`) shows the full set; an agent with `skills:[]` shows none.
    - **AC4**: `hya exec "ulw summarize"` → transcript contains `<hya-injection name="ulw">`/`<ultrawork-mode>`; `hya exec "summarize"` → absent.
    - **AC5** (Round-1 D5): real spawn surface — drive the lead to call `task` with `subagent_type:"deep"` (a `FakeProvider` scripted `task` call in an integration test, OR a real-model `hya exec` that triggers it) → team evidence returns, spawned member ran on `tier-strong`; `subagent_type:"nope"` → that member reported failed, lead unaffected. Capture the evidence envelope.
34. [ ] Cleanup QA artifacts: remove temp `.hya/agents/*`, tmux sessions, temp files.
