...
- Keep hya's architecture (event-sourced engine, permission plane, goal/loop/team). Add config-driven
  agents as a new `hya-core::agent` module; do not rewrite the engine.
...
`ToolRegistry::builtins()` ([tool.rs:68-86](../../../crates/hya-tool/src/tool.rs)) registers **10** tools (review Round 1
corrected 8→9→10): `read, write, edit, ls, glob, find, grep, shell, ask_user` (`AskUserTool`), **and `task`**
(`TaskTool`, [tool.rs:547](../../../crates/hya-tool/src/tool.rs)) — the **subagent-spawn tool**, lead-only
(`ctx.parent_session.is_some()` ⇒ rejected, [tool.rs:599](../../../crates/hya-tool/src/tool.rs)); its JSON schema hardcodes
`subagent_type ∈ {quick,deep,ultrabrain,writing}` ([tool.rs:581](../../../crates/hya-tool/src/tool.rs)) — see §7 for why this
...
`parent_session: Option<SessionId>` ([tool.rs:32-39](../../../crates/hya-tool/src/tool.rs)). Re-verify the live structs at impl time.
...
| C3 | `ToolAllowlist` in hya-core (A/B) vs hya-tool (C) | **hya-tool** (next to `ToolRegistry`), with `schemas_for(&allowlist)` + `require_allowed(&allowlist,name)` | Registry owns schemas+execution; hya-core already depends on hya-tool |
...
crates/hya-core/src/agent/
  ...
  skills.rs      # SkillCatalog + SkillSelection (moved from hya-cli; CLI imports back)
...
crates/hya-tool/src/tool.rs   # + ToolAllowlist, schemas_for, require_allowed
```

## 1. `AgentCore` + `ToolAllowlist`

```rust
// crates/hya-tool/src/tool.rs (or allowlist.rs)
...
```rust
// crates/hya-core/src/agent/core.rs
#[derive(Clone, Debug)]
...
**Parser: reuse `serde_norway`** (workspace dep, used at [config.rs:96](../../../crates/hya-cli/src/config.rs)). A small splitter extracts the `---`-fenced frontmatter; `serde_norway::from_str` parses it; the body is the prompt. The scalar-only `parse_skill` ([skills.rs:10](../../../crates/hya-cli/src/skills.rs)) is insufficient (needs lists/maps) and stays untouched. `#[serde(deny_unknown_fields)]` on frontmatter structs catches typos.

**Agent file** `.hya/agents/<name>.md` (frontmatter + body=prompt):
```yaml
---
...
---
You are hya, a coding agent.
```

**Injection file** `.hya/injections/<name>.md`:
```yaml
---
name: ulw
...
- **Discovery**: builtins (`include_str!`) → global `~/.config/hya/agents/*.md` (honor `$XDG_CONFIG_HOME`) → project `<workdir>/.hya/agents/*.md`. Same for `injections/`. Keyed by frontmatter `name` (must match file stem).
...
- **Advertise** — `request_from_messages` ([engine.rs:474](../../../crates/hya-core/src/engine.rs)): `tools: tools.schemas_for(&agent.allowed_tools)` (was `tools.schemas()`).
- **Execute** — dispatch loop ([engine.rs:325-358](../../../crates/hya-core/src/engine.rs)): `self.tools.require_allowed(&agent.allowed_tools, &tc.name)` (was `self.tools.get(&tc.name)`); disallowed ⇒ `Event::ToolError` "tool not allowed", model self-corrects next round. Unknown-tool error preserved.
...
- **`ulw`**: `{agents:[build,…], models:"*", trigger:keyword, keyword:"(?i)(ultrawork|ulw)", once:true}` — user types "ulw …" ⇒ body injected once as a logged `<hya-injection name="ulw">` System message before the turn.
...
- **Delete** `crates/hya-core/src/category.rs` (`CategoryRegistry`, `CategoryEntry`, `ResolvedCategory`, `build_member_agent`); remove `pub mod category` + re-exports ([lib.rs:18](../../../crates/hya-core/src/lib.rs)). `inject_skills` logic folds into the loader's skills section.
- **Presets**: quick/deep/ultrabrain/writing become builtin agent files `extends: build` with model overrides + body = old `prompt_append`. **Names unchanged**, so `TaskTool`'s `subagent_type` enum `{quick,deep,ultrabrain,writing}` ([tool.rs:581](../../../crates/hya-tool/src/tool.rs)) stays valid as **agent names** (semantics shift category→agent; enum + description kept this task).
- **Runtime spawn path = the real "spawn by name" (Round-1 D6).** Flow: model calls `task` → `TaskTool.execute` builds `Vec<SpawnMember{description,prompt,subagent_type}>` → `ctx.spawner.spawn(..)` ([tool.rs:636](../../../crates/hya-tool/src/tool.rs)) → `SpawnRequest` over mpsc → the **supervisor loop [hya-cli/src/main.rs:200-230](../../../crates/hya-cli/src/main.rs)** drains it and calls `run_team` (the ONLY `run_team` caller). **ACTUAL current behavior** (`spawn_team_supervisor`, [main.rs:199-238](../../../crates/hya-cli/src/main.rs)): the supervisor **ignores `subagent_type`** and clones a single `base: AgentSpec` into every `MemberSpec` (using `m.prompt` as the directive) — so `task subagent_type:deep` currently runs on the **base build model**. Category routing is **NOT wired into the live spawn path**; `CategoryRegistry`/`build_member_agent` are exercised ONLY by `tests/category_routing.rs`, never at runtime. **Migrate the supervisor** to hold an `Arc<AgentCatalog>` and map each `SpawnMember.subagent_type` → `MemberSpec { agent: AgentName::new(subagent_type) }`, passing the catalog into `run_team` (resolution + unknown⇒`Failed` happen in `run_member`). This is a behavior **improvement**: spawned members finally get their preset's model. `SpawnerPlane` / `ToolCtx.spawner` / `engine.with_spawner` ([engine.rs:42,79,343](../../../crates/hya-core/src/engine.rs)) are UNCHANGED.
- **`run_team`/`run_member`** ([subagent.rs:76,53](../../../crates/hya-core/src/subagent.rs)): `run_team(engine, lead, catalog: Arc<AgentCatalog>, specs, cancel)`; `run_member` resolves `catalog.require(member_name)?` → `AgentCore` (workdir from lead). Resolution failure ⇒ `MemberEvidence{status: Failed, summary}`.
...
Rename consumers that hold/read an agent VALUE (mechanical `AgentSpec`→`AgentCore`, no catalog needed): [engine.rs](../../../crates/hya-core/src/engine.rs) (`run_turn`, `projection_to_messages`, `request_from_messages`), [completion.rs](../../../crates/hya-core/src/completion.rs) (`run_goal`, `LeadTurnExecutor.agent`), [loop_mode.rs](../../../crates/hya-core/src/loop_mode.rs) (`run_loop`, `WorkerSessionExecutor.agent`), [hya-server/src/lib.rs](../../../crates/hya-server/src/lib.rs) (`AppState.agent: Arc<AgentCore>`), [hya-cli/src/tui.rs](../../../crates/hya-cli/src/tui.rs) (`run`, `spawn_turn`, `/model` re-resolve). Sites with API changes: [subagent.rs](../../../crates/hya-core/src/subagent.rs) (`MemberSpec.agent: AgentName`; `run_member`/`run_team` take `catalog`), [hya-cli/src/main.rs](../../../crates/hya-cli/src/main.rs) (`agent_with_model`→`AgentCatalog`; **supervisor loop :200-230** holds the catalog + resolves `subagent_type`). **`run_team` is called ONLY by the supervisor** ([main.rs:217](../../../crates/hya-cli/src/main.rs)) — completion/loop_mode use `run_turn` on a single agent ⇒ rename only. Tests: turn_loop, subagent, goal_loop, category_routing, server/api construct `AgentSpec` literals → add new fields / switch type. Transitional `pub type AgentSpec = AgentCore;` keeps concurrent worktree code compiling; **removed in final cleanup** (`rg AgentSpec crates/` ⇒ zero). `cargo build --workspace` catches every miss.
...
- **`/model` slash command** mutates `agent.model` ([tui.rs](../../../crates/hya-cli/src/tui.rs)) → must re-resolve model-dependent injections; covered by test.
...
D5 FAIL: AC3/AC5 lack real-surface QA → **fixed**: added live `hya exec` skills QA + live `task`-tool spawn-by-name QA. [implement Wave 9]
...
D1, D2, D4, D5, D6 **PASS**. D3 **FAIL**: design misstated current supervisor behavior — `spawn_team_supervisor` ([main.rs:199-238](../../../crates/hya-cli/src/main.rs)) actually **ignores `subagent_type`** and clones `base`; it does NOT use `CategoryRegistry`/`build_member_agent` (test-only). → **fixed**: §7/§8 + implement 27c now state real behavior; migration reframed as *wiring* the currently-ignored `subagent_type` → catalog resolution (a behavior improvement, not a replacement).

### Round 3 — codex `gpt-5` (cross-family) — VERDICT: PASS
All D1–D6 **PASS**. D3 confirmed against source (`spawn_team_supervisor` ignores `subagent_type` + clones base, [main.rs:199](../../../crates/hya-cli/src/main.rs); `TaskTool` [tool.rs:547,581](../../../crates/hya-tool/src/tool.rs); `CategoryRegistry`/`build_member_agent` test-only). **Cross-model gate cleared — plan is execution-ready, pending user `task.py start`.**
