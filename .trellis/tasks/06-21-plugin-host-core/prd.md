...
- **A-R1 (R1).** A hook-dispatch trait seam in `yaca-core` (e.g.
...
- **A-R2 (R2).** A `yaca-plugin` crate: protocol types + a host/manager that
...
- **A-R6 (R7).** `plugins:` config section + native dir-scan
  ...
  per-plugin `enabled`. A single bootstrap in `yaca-cli` wires the host before the
...
- [ ] **A-AC1 (AC1).** A native example plugin's `tool.execute.before` fires in a
      ...
      blocks the tool — proven by test + a `yaca exec` QA transcript.
...
- [ ] **A-AC4 (AC5).** Killing the plugin mid-turn neither crashes nor hangs yaca;
