# Implement - OpenCode-compat Bun adapter (Child C)
...
- Child A protocol/host is merged and exposes `kind: opencode` plugin process
...
4. Add a Rust-side command resolution function for `kind: opencode` that checks
...
2. Implement `initialize` handling that reads env/options, loads OpenCode plugins,
   and replies with:
   - `protocol_version: 1`
...
- RED: `bun test loader.discovery` — adapter options parse OpenCode's
  `plugin?: Array<string | [string, options]>`; discovery returns candidates from
  `${directory}/.opencode/{plugin,plugins}/*.{ts,js}` and
  `${XDG_CONFIG_HOME:-~/.config}/opencode/{plugin,plugins}/*.{ts,js}`.
...
1. Implement `init-context.ts` to synthesize the exact OpenCode `PluginInput`:
...
## 6. Phase 6 - OpenCode `tool:` registration and `tool/call`
...
## 7. Phase 7 - Rust integration for `kind: opencode`
...
Rollback point: disable the `kind: opencode` resolver and require explicit
commands for manual adapter testing.

## 8. Phase 8 - Live QA with real OpenCode plugin
...
2. Configure one real off-the-shelf OpenCode plugin using either:
   - a public `.opencode/plugins` plugin that implements `tool.execute.before`,
     for example the Trellis OpenCode injection plugin template, or
   - a public OpenCode event plugin such as `qeesung/tmux-scout` or CodeIsland.
...
- OpenCode loader drift beyond v1.17.9: mitigate by version pin and loader tests
