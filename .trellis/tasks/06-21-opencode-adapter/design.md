...
> Child C delivers the OpenCode compatibility layer only. The yaca plugin
...
- Parent requirement R8 defines the OpenCode adapter as a bundled Bun child that
  loads JS/TS plugins, maps OpenCode `Hooks` to the yaca protocol, points an SDK
  `client` at `yaca serve`, and runs OpenCode's plugin discovery and npm install
...
crates/yaca-plugin-opencode/
  ...
  adapter/
    ...
    src/hooks.ts             # yaca hook <-> OpenCode hook translation
    src/sdk-client.ts        # OpenCode-SDK-shaped yaca client shim
...
plugins:
  opencode:
    ...
    command:
      ...
      - run
      - --cwd
      - ${YACA_BUNDLED_OPENCODE_ADAPTER}/adapter
      - src/main.ts
    timeout_ms: 1000
    env:
      YACA_OPENCODE_TRACE: "0"
    options:
      plugin:
...
- `YACA_PROJECT_ID`: stable yaca project/workspace id if available, else a hash of
  `YACA_WORKTREE`.
- `YACA_AGENT`, `YACA_MODEL`: defaults used when mapping OpenCode SDK
  `session.create` to yaca's `POST /sessions` DTO.
- `YACA_OPENCODE_OPTIONS_JSON`: serialized `options` from the yaca plugin config.
...
5. Replies to `initialize` with `plugin.kind = "opencode"`, one declared yaca
   hook for every supported OpenCode hook present, and one yaca tool declaration
...
- `client`: an OpenCode-SDK-shaped object backed by `yaca serve` and local shims.
...
own SDK-shaped client because yaca's HTTP paths are not OpenCode's generated SDK
paths.
...
The adapter runs all OpenCode hooks for a given yaca hook sequentially in
OpenCode load order, feeding the mutated output from one plugin into the next.

| yaca v1 hook/protocol surface | OpenCode hook key | Adapter input | Adapter output / yaca outcome | Notes |
|---|---|---|---|---|
| `event` notification | `event` | `{ event }`, where `event` is an OpenCode-style event union | No yaca reply | Convert yaca `Envelope` to closest OpenCode event. Unknown yaca events may be emitted as `yaca.<type>` only when trace compatibility mode is enabled; default off for off-the-shelf plugins. |
...
| Plugin tool registration in `initialize.tools` | `tool` object on returned `Hooks` | OpenCode `ToolDefinition` from `tool({ description, args, execute })` | yaca `tools` declarations in `initialize` and `tool/call` replies | Convert Zod args to JSON schema for yaca `ToolSchema`; invoke `execute(args, context)` on `tool/call`. Source: [OpenCode tool helper](/tmp/opencode-src/packages/plugin/src/tool.ts:45), [yaca ToolSchema](/chivier-disk/yanweiye/Projects/yaca/crates/yaca-proto/src/model.rs:46). |
| `goal.evaluate` | none | n/a | `defer` | No OpenCode equivalent; leave to yaca. |
| `loop.verifier` / `loop.planner` | none | n/a | `defer` | No OpenCode equivalent; leave to yaca. |
...
map through a yaca v1 hook above. Source: [Child C scope](/chivier-disk/yanweiye/Projects/yaca/.trellis/tasks/06-21-opencode-adapter/prd.md:38).

## 4. Outcome Semantics At The Adapter Boundary

For each blocking yaca hook request:

1. Decode the yaca request into the OpenCode `input` and mutable `output` object.
...
5. **Token-delta firehose into `event`.** The adapter's `event` handler is
   ...
   sampled warn), so a slow OpenCode `event` handler never back-pressures yaca's
   `emit`.
...
| OpenCode SDK method/group | Adapter backing | Current yaca endpoint | Status / gap |
|---|---|---|---|
| `client.session.create({ body: { parentID?, title? } })` | Translate to yaca create session using adapter defaults for agent/model/workdir | `POST /sessions` | Supported. yaca requires `agent`, `model`, `workdir`; adapter supplies them from env/context. Source: [CreateSessionRequest](/chivier-disk/yanweiye/Projects/yaca/crates/yaca-proto/src/api.rs:6), [OpenCode SessionCreateData](/tmp/opencode-src/packages/sdk/js/src/gen/types.gen.ts:2082). |
| `client.session.prompt({ path: { id }, body: { parts, ... } })` | Flatten OpenCode text parts into one yaca prompt string | `POST /sessions/:id/prompt` | Supported for text-only parts. File/agent/subtask parts need a future yaca prompt DTO, not a protocol change. Source: [PromptRequest](/chivier-disk/yanweiye/Projects/yaca/crates/yaca-proto/src/api.rs:20), [OpenCode SessionPromptData](/tmp/opencode-src/packages/sdk/js/src/gen/types.gen.ts:2588). |
| `client.session.messages({ path: { id } })` | Prefer future yaca projection endpoint; interim can replay events and reduce in adapter tests only | none direct; current `GET /sessions/:id/events` | Gap. Smallest yaca-server follow-up: `GET /sessions/:id/messages` returning projected `Message[]` or OpenCode-compatible `{ info, parts }[]`. Source: [Event replay endpoint](/chivier-disk/yanweiye/Projects/yaca/crates/yaca-server/src/lib.rs:104), [OpenCode SessionMessagesData](/tmp/opencode-src/packages/sdk/js/src/gen/types.gen.ts:2548). |
...
| `client.file.read/list/status`, `client.find.text/files/symbols` | Local filesystem and `rg`/glob shim constrained to `directory` | none needed | Optional adapter-local support for plugins that inspect files. Must not bypass yaca permissions for yaca tool execution; this only serves plugin init/runtime calls. Source: [FileReadData](/tmp/opencode-src/packages/sdk/js/src/gen/types.gen.ts:3231), [FindTextData](/tmp/opencode-src/packages/sdk/js/src/gen/types.gen.ts:3138). |
| `client.config.get/providers`, `client.provider.list/auth` | Minimal read-only shape from yaca config and adapter defaults | none | Gap for exact provider/auth compatibility. Return documented `UnsupportedSdkMethodError` unless a target plugin requires it. OpenCode auth/provider hooks are out of Child C scope. |
| `client.tool.ids/list` | Future yaca tool listing endpoint or Child B registry export | none | Gap. If a target plugin needs it, smallest follow-up is `GET /tools` with model/provider query returning yaca `ToolSchema[]`; otherwise unsupported. Source: [OpenCode ToolListData](/tmp/opencode-src/packages/sdk/js/src/gen/types.gen.ts:1981). |
...
- `cargo build --workspace`, `cargo test --workspace`, yaca core, yaca server,
  and yaca CLI without enabled OpenCode adapter must not require Bun.
...
- OpenCode command/shell/tool-definition hooks unless later yaca hooks exist.
...
- The strongest yaca-side risk is SDK surface mismatch, not hook dispatch. The
  ...
  generated OpenCode SDK directly on yaca's current paths.
