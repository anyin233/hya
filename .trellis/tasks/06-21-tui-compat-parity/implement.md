...
```sh
python3 ./.trellis/scripts/task.py start 06-21-tui-compat-parity
```
...
- Implement compat-inspired row rendering:
...
- Added compat-style custom slash command loading from:
...
- Added `/tools` and `/mcp` status dialogs for builtin tools and the current
  MCP gap.
- Applied compat custom command `agent` and `model` frontmatter when a custom
