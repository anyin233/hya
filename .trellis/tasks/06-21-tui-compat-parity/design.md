# Design: compat-inspired Rust TUI refactor

## Scope

This task refactors the existing Rust/ratatui TUI toward the Compat TUI's
...
Compat's current TUI is a componentized terminal app:
...
- `widgets/tools.rs`
  ...
  - Map known yaca tool names (`read`, `write`, `edit`, `glob`, `grep`,
    `shell`/`bash`, `task`, unknown) to compat-style labels/icons.
- `widgets/sidebar.rs`
...
This gives the user the visible context density they expect from Compat while
...
- Compat has richer SDK data than yaca currently projects. Sidebar context
