# Design: hya — Rust Multi-Agent Coding Agent
...
hya is **one workspace, two binaries**: a `core` **server** (owns all agent
...
9 crates; acyclic graph; `hya-proto` is dependency-light and shared by client +
server so the TUI never compiles sqlx/tokio-heavy code it doesn't need.

```
hya-proto     wire types: ids, Message, Part, Event, Envelope, ToolSchema. (serde/uuid/time only)
hya-provider  Provider/Protocol/Route traits + Anthropic/OpenAI/OpenAI-compat impls + FakeProvider
hya-tool      Tool trait, JSON-schema, ToolRegistry, PermissionPlane
hya-store     sqlx SQLite: event log + projection + replay
hya-core      domain: SessionEngine, AgentLoop, TeamOrchestrator (module), GoalEngine (module), EventBus
hya-server    axum HTTP/SSE server over hya-core
hya-client    thin SDK: typed HTTP/SSE client (shared by TUI; future remote tooling)   [architect's wire-stability point]
hya-render-tui       ratatui client over hya-client
hya-cli       `hya` umbrella binary: spawns server + TUI; `serve`, `-p` headless, `tail-session`
```

Dependency direction: `cli → {server, tui}`; `server → core`; `tui → client → proto`;
`core → {provider, tool, store, proto}`; everything → `proto`. `hya-core` is
HTTP-agnostic (testable without a server). team + goal are **modules inside
`hya-core`** (not separate crates) because both are tightly coupled to
...
## 4. Provider abstraction (`hya-provider`) — the keystone
...
- **Auth** via `secrecy::SecretString`, lives ONLY in provider impls, never in
  `hya-core`, never logged. Config `${env:NAME}` expansion.

## 5. Tool system + permissions (`hya-tool`)
...
## 6. Sessions & persistence (`hya-store`) — event-sourced + projected
...
## 7. Agent turn loop (`hya-core::AgentLoop`)
...
## 8. Team orchestration (`hya-core::team`) — full omo parity (D4)
...
## 9. Completion engine (`hya-core::completion`) — goal + loop (D4, D5)
...
`hya -p "/loop …"`. One completion run (goal XOR loop) per session at a time.
...
- **Config**: `$XDG_CONFIG_HOME/hya/config.toml` (providers, agents, categories,
  goal caps, permissions). Server binds `127.0.0.1:0`; port written to
  `$XDG_RUNTIME_DIR/hya/port` for client discovery; no external exposure without
  explicit `--bind`.
...
4. **Crate count — 9 (middle path).** rust-impl's 8 + a `hya-client` SDK crate
   (architect's wire-stability/remote-readiness point); team+completion stay as
   `hya-core` modules (rust-impl) because they're coupled to SessionEngine.
