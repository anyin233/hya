...
> research under `research/` (compat, omo team mode, Claude Code `/goal`).
...
compat's split, in Rust. **Every provider normalizes to the canonical `Event`
...
Two layers (compat §4). **Canonical** `ToolSchema` (name + description +
...
**Permission plane** (compat's control plane): `(Action, Resource)` evaluated
