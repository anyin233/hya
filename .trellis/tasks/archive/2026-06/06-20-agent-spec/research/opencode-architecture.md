...
Source: librarian deep-dive of the active `anomalyco/opencode` tree
(SHA `bd84c32860a7650965419716214196adbbb00e2f`; this is what `opencode.ai`
currently ships; `sst/opencode` resolves to the same tree). All paths below are
in that repo.

## TL;DR for our Rust design

opencode is a **Bun/TypeScript monorepo** with a **client/server** split, an
...
- `packages/opencode` — CLI, runtime, **server**, tool registry, provider orchestration
...
- Running `opencode` starts a **TUI client + a server**; the TUI talks to the
...
- **Registry** (`packages/opencode/src/tool/registry.ts`) assembles built-ins
