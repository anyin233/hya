...
**Goal:** Add a tag-triggered GitHub Actions release workflow that builds and publishes the `yaca` binary with release notes read from newest-only `CHANGELOG.md`.
...
- **Pass condition:** A `v0.0.0`-style tag would build `target/x86_64-unknown-linux-gnu/release/yaca`, package `yaca-0.0.0-x86_64-unknown-linux-gnu.tar.gz`, produce `SHA256SUMS`, and publish a GitHub Release body from `CHANGELOG.md`.
- **Verification:** `actionlint .github/workflows/release.yml` if available; otherwise document tool absence. Run `cargo build --release --bin yaca --target x86_64-unknown-linux-gnu`, package to a temporary directory, extract, run packaged `./yaca --version`, and run packaged `./yaca --help`.
...
- **Pass condition:** The new release workflow does not modify `.github/workflows/ci.yml` or Rust production code, and `cargo build --release --bin yaca --target x86_64-unknown-linux-gnu` succeeds locally.
...
- Env: `CARGO_TERM_COLOR: always`, `RUSTFLAGS: "-D warnings"`, `BINARY_NAME: yaca`, `TARGET: x86_64-unknown-linux-gnu`
...
4. Validate tag version shape using a semver regex, then validate it using `cargo metadata --no-deps --format-version 1` and Python JSON parsing for package `yaca-cli`.
5. Validate `CHANGELOG.md` with `test -s CHANGELOG.md` and require the first heading to equal `# <version>`.
6. Build: `cargo build --release --locked --bin yaca --target "$TARGET"`.
7. Package a versioned directory under `dist/`, copy the binary and `README.md`, create `.tar.gz`, and create `SHA256SUMS`.
8. Extract the `.tar.gz` to a temp directory; run packaged `yaca --version` and grep the version; run packaged `yaca --help`.
...
- `cargo build --release --locked --bin yaca --target x86_64-unknown-linux-gnu`
...
D3 PASS  # All cited surfaces exist and resolve: ci.yml uses the same actions [.github/workflows/ci.yml:15-19]; yaca bin [crates/yaca-cli/Cargo.toml:8-10]; workspace version 0.0.0 [Cargo.toml:8]; `yaca --version` is derived by clap from CARGO_PKG_VERSION [crates/yaca-cli/src/main.rs:45-50]; assumption (Linux x86_64 only) is named [prd.md:53].
...
- `RUSTFLAGS='-D warnings' cargo build --release --bin yaca --target x86_64-unknown-linux-gnu`: PASS.
- Local release surface smoke test: packaged `target/x86_64-unknown-linux-gnu/release/yaca` into `yaca-0.0.0-x86_64-unknown-linux-gnu.tar.gz`, generated and verified `SHA256SUMS`, extracted the archive, ran packaged `yaca --version`, and ran packaged `yaca --help`: PASS.
...
- `RUSTFLAGS='-D warnings' cargo check --bin yaca --target x86_64-unknown-linux-gnu`: PASS after a transient shared-worktree Rust mismatch resolved.
- `RUSTFLAGS='-D warnings' cargo build --release --bin yaca --target x86_64-unknown-linux-gnu`: PASS.
- Local release surface smoke test: rebuilt `yaca-0.0.0-x86_64-unknown-linux-gnu.tar.gz`, verified `SHA256SUMS`, extracted it, ran packaged `yaca --version`, and ran packaged `yaca --help`: PASS.
...
- `RUSTFLAGS='-D warnings' cargo build --release --locked --bin yaca --target x86_64-unknown-linux-gnu`: PASS.
- Local release surface smoke test after hardening: rebuilt `yaca-0.0.0-x86_64-unknown-linux-gnu.tar.gz`, verified `SHA256SUMS`, extracted it, ran packaged `yaca --version`, and ran packaged `yaca --help`: PASS.
- `cargo fmt --all --check && cargo clippy --workspace --all-targets -- -D warnings && cargo test --workspace`: PASS; test suite reported all executed tests passing with one pre-existing ignored worktree test.
