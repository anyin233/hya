# Design — hya pi-parity (Tier 3, phased)

## Guiding principles

- **Keep hya's architecture.** Event-sourced engine, permission plane, goal/loop/team
...
2. **Headless auto-responder** (`hya-cli`, new `permission.rs`): a spawned task that
...
3. **Interactive TUI responder** (`hya-cli/src/tui.rs` + `hya-render-tui`): the event loop
...
In `hya-tool/src/tool.rs`, add to `ToolRegistry::builtins`:
...
- `crates/hya-cli/src/main.rs` — keep receiver; add `--yolo`; spawn responder per mode.
- `crates/hya-cli/src/permission.rs` (new) — `PermissionPolicy`, auto-responder, path containment.
- `crates/hya-cli/src/tui.rs` — select on asks; approval keybinds.
- `crates/hya-render-tui/src/lib.rs` — `AppState.pending_permission`, overlay render.
- `crates/hya-tool/src/tool.rs` — `LsTool`, `FindTool`, edit ambiguity guard.

### Acceptance (Wave 1)
- AC1: `hya exec "create file /workdir/x.txt …"` (real model) writes the file; outside-workdir write is rejected without `--yolo`; `--yolo` allows it.
...
- New `hya-core` system-prompt builder: base persona + environment preamble
...
- `hya-cli` slash registry: `/help /model /clear /new /exit` (extensible map).
...
- Add Google provider decoder in `hya-provider` (Gemini generateContent/SSE),
...
- `print`/JSON mode: `hya exec --json` emits the `Envelope` event stream as JSONL
  (already serializable) instead of rendered transcript.
- RPC mode: `hya rpc` reads JSONL requests on stdin (prompt/create/replay), writes
