# yaca: broad feature parity with pi

## Goal

Bring **yaca** (Rust multi-agent coding agent, ~5.7K LOC) to broad (Tier 3)
*functional* parity with **pi** (`earendil-works/pi`, ~200K LOC TS coding agent),
while preserving yaca's existing differentiators. "Parity" = a user can do the
same core coding-agent work in yaca that they can in pi; it is NOT a line-by-line
port of pi's TypeScript.

## User value

Today yaca is a clean foundation that **cannot actually code**: edit/write/shell
are blocked in every mode. After this work yaca is a usable coding agent with
...
- Tools (`yaca-tool/src/tool.rs` `ToolRegistry::builtins`): `read, write, edit, glob, grep, shell` (6).
- **CRITICAL BUG**: `yaca-cli/src/main.rs::build_session_engine` drops the permission
  ...
  → tool fails. Verified live: `yaca exec "write hello to test.txt"` returns a permission error.
...
- **Extensions**: TS plugin runtime (N/A to Rust yaca — out of scope as a port).
...
- R1. yaca must execute edit/write/shell successfully in TUI (with approval UX) and
...
- R10. yaca's existing differentiators (goal/loop/team/worktrees/categories) keep
  working (regression-free) throughout.

## Acceptance criteria

- [ ] AC1. `yaca exec "create file X with content Y"` against a real model actually
      creates the file; `yaca` TUI prompts for approval on a mutating tool and
...
- Porting pi's TypeScript **extension runtime** (Rust has no TS plugin host) — yaca's
...
- Becoming pi: yaca keeps its event-sourced, permissioned, multi-agent architecture.

## Resolved decisions

- D1 (Wave 1): **Keep & fix yaca's permission plane.** TUI shows interactive
...
- D2: Keep yaca's architecture & differentiators (event-sourced, permissioned,
