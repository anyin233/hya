...
**Goal:** Make Compat agent/model `variant:` values such as `max` drive real provider extended-thinking requests in yaca, with provider/model validity and backward-compatible defaults.

**Architecture:** Resolve Compat variant/options at the server/native request boundary into a typed `ReasoningEffort`, then normalize that effort in `yaca-provider` against provider family + model id before encoding wire-specific request fields. Keep event/projection behavior unchanged: providers emit canonical reasoning events, and existing TUI/Compat surfaces render them.

**Tech Stack:** Rust 2024 workspace; `serde_json::Value` for the existing Compat config boundary; yaca event-sourced engine; provider protocols in `yaca-provider`; Compat-compatible HTTP surface in `yaca-server`.
...
- Reuse existing compat config/agent catalog loaders instead of adding an unrelated configuration system.
...
- `AgentSpec.reasoning` exists and `request_from_messages()` forwards it into `CompletionRequest`, but runtime/server constructors leave it unset for Compat variants.
- `yaca-server` parses `AgentEntry.variant` and `AgentEntry.options`, and `model_ref.rs` preserves `#variant` in Compat model refs, but `reference.rs::session_agent_with_guidance()` only copies selected agent `prompt` and `name`.
...
pub enum AnthropicThinking {
    ...
    /// Source-gated Compat adaptive mode. Implement this encoder branch only
...
- `ReasoningEffort::None` is still useful because Compat's vocabulary includes `none`; when an option bundle explicitly says `none`, it should override weaker defaults. `Option<ReasoningEffort>::None` means “no configured reasoning setting”. `Some(ReasoningEffort::None)` means “explicitly disable reasoning”.
...
| Anthropic adaptive | `claude-opus-4-7*`, `claude-opus-4-8*`, `claude-sonnet-4-6*` and later known adaptive models | `low,medium,high,xhigh,max` | **Source-gated.** Compat research reports adaptive thinking for these models, but implementation must verify the exact Anthropic wire shape before emitting it. If not verified, normalize to bounded budget mode (`low/medium -> high`, `xhigh/max -> max`) so yaca never sends an undocumented body. | `minimal -> low`; `none -> omit`; unknown high levels clamp to max |
...
- Populate it from Compat provider model metadata if available in `opencode.json` / `/global/config` (`provider.<id>.models.<id>.limit.output` or equivalent value object).
...
- If `Adaptive { effort }`, require a failing encoder test that asserts the exact Compat-backed Anthropic wire object. If the implementer cannot source that contract, do not emit adaptive; normalize to `Budget` via the bounded helper instead. This fail-closed rule avoids production 400s on undocumented request bodies.
...
- Include `includeThoughts:true` if Compat parity requires it for thought parts from Google.
...
### 1.5 Compat config and variant resolution

Add a focused resolver module:

**Create:** `crates/yaca-server/src/compat/reasoning_options.rs`
...
6. Extract a typed effort from Compat-compatible option keys:
...
This module deliberately parses only reasoning-related keys. Generic provider-option pass-through remains out of scope from the PRD, except for reasoning fields needed by Compat variants.

### 1.6 Agent variant -> reasoning on the native path

The native path is `hya -> hya-yaca -> in-process yaca-server Compat endpoints -> yaca-core -> yaca-provider`, so resolution must happen in the Compat server agent preparation layer, not in the TUI renderer.

Modify `crates/yaca-server/src/compat/reference.rs`:
...
- If `entry.model` is present and `entry.variant` is present, construct `ModelRef` as `model#variant` only for Compat display/session metadata if needed; actual reasoning should come from `resolve_reasoning()` so provider encoders never depend on the suffix after `HttpProvider::stream()` strips it.
...
| Config.yaml-only user | unchanged; no Compat variants loaded | `yaca-app::config` untouched except fallback parse of `#variant` | runtime/config test |
...
### Task 3: Add Compat reasoning option resolver
...
- Create: `crates/yaca-server/src/compat/reasoning_options.rs`
- Modify: `crates/yaca-server/src/compat/mod.rs`
- Test: `crates/yaca-server/tests/compat_reasoning_variants_api.rs` or `crates/yaca-server/tests/compat_agent_config_api.rs`
...
7. Run: `cargo test -p yaca-server compat_reasoning_variants -- --nocapture`.
...
- Modify: `crates/yaca-server/src/compat/reference.rs`
- Modify if needed: `crates/yaca-server/src/compat/session_v2.rs`
- Modify: `crates/yaca-app/src/runtime.rs`
- Test: `crates/yaca-server/tests/compat_reasoning_variants_api.rs`
...
| --- | --- | --- | --- |
| `reasoning_effort_parses_compat_vocab` | `crates/yaca-provider/tests/conformance.rs` | AC2 | `none,minimal,low,medium,med,high,xhigh,max` parse; unknown returns `None` |
...
| `anthropic_high_and_max_budgets_match_compat` | `crates/yaca-provider/tests/conformance.rs` | AC2, AC3 | `high=16000`, `max=31999` bounded by output limit |
...
| `compat_model_variant_bundle_resolves_reasoning` | `crates/yaca-server/tests/compat_reasoning_variants_api.rs` | AC3 | `provider.12th.models.claude-opus-4-8.variants.max` produces `ReasoningEffort::Max` and Anthropic body budget 31999 |
| `agent_frontmatter_variant_sets_agent_spec_reasoning` | `crates/yaca-server/tests/compat_reasoning_variants_api.rs` | AC4 | `.opencode/agent/*.md` `variant:max` sets `AgentSpec.reasoning=Some(Max)` |
| `agent_unknown_reasoning_effort_option_is_honored` | `crates/yaca-server/tests/compat_reasoning_variants_api.rs` | AC4 | `reasoningEffort:xhigh` in frontmatter extra/options resolves to `XHigh` |
...
1. **Risk: generic Compat option bundles exceed typed `ReasoningEffort`.**
...
3. **Risk: Anthropic output limit unavailable on native config path.**
   - Mitigation: introduce `Capabilities.max_output_tokens`; populate from Compat config when present; use `32_000` fallback and `31_999` budget when absent.
...
7. **Risk: `ReasoningEffort::None` inside `Option` is confusing.**
   - Mitigation: document semantics in `lib.rs`: outer `Option` = configured/unconfigured, inner `None` variant = explicit Compat disable.

---

## 6. Open Questions

1. **Should this task implement generic provider-option pass-through?** The PRD says non-reasoning options are out of scope, but Compat variants are generic option bundles. This plan recommends parsing only reasoning-related keys to keep scope bounded while satisfying the reasoning ACs.
...
3. **Where should Compat provider model `limit.output` be represented?** This plan uses `Capabilities.max_output_tokens`. If the team wants richer model metadata later, extend `ProviderModel` rather than adding ad hoc config reads to encoders.
...
5. **Do Google thought parts require `includeThoughts:true` for all reasoning levels?** The provider encoder should match Compat once verified against live Gemini responses.
...
- Scope discipline: source changes are limited to provider normalization, Compat reasoning resolution, runtime fallback, and tests.
...
```text
D1 PASS: AC1–AC6 each mapped to tasks/tests in [plan-draft-deep.md:459-473]; AC1 manual tmux verification in Task 6, AC2/AC5 in conformance tests, AC3/AC4 in compat_reasoning_variants_api, AC6 via §0 gate command.
