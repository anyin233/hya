# Compat reasoning / variant schema (research)

Source: Compat (sst/compat, repo mirror `anomalyco/compat`) dev HEAD
...
- **Anthropic**: `thinking: { type: "enabled", budget_tokens: N }` (key is `budget_tokens` on the wire; Compat config uses `budgetTokens`). Adaptive: `thinking:{type:"adaptive"}` + `effort`. `anthropic-messages.ts#L491-L501`.
...
- Read `provider.<id>.models.<id>.{options,variants}` from opencode.json (yaca-server compat layer already loads opencode.json/jsonc via `global.config()`).
