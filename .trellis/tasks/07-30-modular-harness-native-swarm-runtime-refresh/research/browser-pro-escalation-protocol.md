...
### `CONSULT-2026-08-03-DURABLE-ADMISSION-CERTIFICATION-27`

```yaml
consultation_id: CONSULT-2026-08-03-DURABLE-ADMISSION-CERTIFICATION-27
date: 2026-08-03
scope: narrow 0.34.12 owner override for the capacity authority required by truthful R10 certification
status: Pro-arbitrated, Mac source-corrected, Sol source-validated, authoritative
choice: A
supersedes: only the literal certification-only wording for 0.34.12 in Consult26 and older planning
retains: completed 0.34.11 sidecar release, 0.34.13 updater numbering, all catalog/binding/permission/epoch/stop/sidecar invariants
```

The owner keeps the release numbering and authorizes exactly one missing product prerequisite inside 0.34.12: the minimal durable 100-active/156-non-active/256-envelope admission authority, followed by the complete integrated R10 certification matrix at the same exact release SHA. A failing evidence-only matrix is neither certification nor a releasable 0.34.12. No other feature work enters the release; 0.34.13 remains only the independent updater.

The 100 active slots count subagent work items only. Lead/root work is outside both the 100 active slots and the 256 subagent envelopes. Each nonterminal subagent owns one durable envelope. `Accepted` and `Started` consume the active class; `Queued` and `Waiting` consume the 156 non-active class. `Accepted` means the store durably awarded an active lease before task start; `Started` means the task crossed its start barrier. The invariants are active <= 100, non-active <= 156, and total nonterminal <= 256. Item 257, or a batch whose final vector exceeds either class, rejects before any member row, budget debit, actor claim, Tokio task, sidecar, or provider allocation.

Batch admission is one transaction and all-or-none; members promote independently. Promotion is oldest eligible by original admission sequence within a root and durable bounded round-robin across roots, with never-promoted roots first, deterministic admission-sequence/stable-root ties, ineligible-row skipping, and no wall-clock aging or timer. `Waiting -> Queued` retains the original sequence and reacquires an active lease only through this scheduler. Releasing N `Accepted`/`Started` leases promotes at most N eligible rows in the same bounded store command; cancelling only `Queued`/`Waiting` rows promotes none.

A parent wait is one atomic capacity transition: admit the child batch and move the parent `Started -> Waiting` only if the final 100/156 vector fits. Waiting releases the active lease but retains one non-active envelope; dependency completion moves it to `Queued`, never directly back to running. Capacity failure leaves the parent `Started` and admits no child.

The existing 128 provider-stream capacity becomes two hard, non-borrowing semaphores: 100 general permits for promoted subagent streams and 28 reserved permits for lead/root, control, and recovery orchestration. A subagent must hold a durable `Accepted`/`Started` lease before acquiring a general permit. Root uses only the reserved class and never increments admission counts. Semaphores are live execution limits reconstructed after restart, never durable ordering or acceptance authority.

The existing `admission_journal` remains the only durable queue/lease authority and is extended additively with internal `Queued` and `Waiting` states plus the minimum root, batch, original-admission, promotion-order, and actor/lease-generation data required by deterministic fairness and recovery. No second queue table, projection, scheduler, runtime, catalog, mailbox, or generic resource framework is authorized. Admission, promotion, release, parent wait, and recovery use bounded writer-ordered SQLite transactions and return committed promoted identities; app allocates tasks only from committed `Accepted` results, and the task remains blocked until `Started` commits. Stale `Accepted` requeues; stale `Started` aborts under existing fencing; `Queued`/`Waiting` persist and are rescheduled deterministically. Stable Event and AgentName bytes remain unchanged, and the process-local governor becomes only an execution cache/permit mechanism.

The non-skippable RED order is: (1) migration/state machine/counts/idempotent recovery; (2) atomic 100/156/257 and batch rollback; (3) zero-task queue, one launch instruction, start barrier, pre-start recovery; (4) persisted bounded round-robin fairness and restart-stable order; (5) atomic parent suspension/child batch and fair reacquisition; (6) completion/cancellation promotion and exactly-once accounting; (7) non-borrowing 100+28 provider partition and root progress; (8) mixed-state restart convergence; (9) complete exact-SHA R10 certification including 0.34.11 regressions and frozen resource/SLO thresholds.

Explicit exclusions are: no evidence-only certification, renumbering, updater work, root inclusion in 100/256, provider borrowing, partial batch admission, gang scheduling, wall-clock aging, polling/watcher/retry daemon, application-global lock, queued task/process/sidecar/provider/actor allocation, replay of stale `Started`, second durable authority, generic resource-vector framework, Event/AgentName byte changes, or changes to RuntimeSnapshot catalog ownership, captured TurnBinding, PermissionPlane, resident stop linearization, actor fencing, or 0.34.11 sidecar behavior.

### `CONSULT-2026-08-03-RUNTIME-SEMANTIC-FINGERPRINT-28`

```yaml
consultation_id: CONSULT-2026-08-03-RUNTIME-SEMANTIC-FINGERPRINT-28
date: 2026-08-03
scope: cross-process identity and reconstruction of restart-reconstructable 0.34.12 admission envelopes
status: Pro-arbitrated, Mac source-corrected, Sol source-validated, authoritative
choice: A
supersedes: Consult27's provisional use of ConfigGeneration equality as a cross-process acceptance predicate
retains: one RuntimeSnapshot and BundleCatalog authority, one admission journal, captured-binding semantics, no historical catalog retention, REDs 1-2
```

`ConfigGeneration` and BundleRegistry generation are ordinals/change detectors, not cross-process semantic identities. Recovery eligibility uses only equality of a private, versioned `RuntimeSemanticFingerprintV1`; diagnostic generation is persisted but never participates in acceptance. The fingerprint is computed after bundle refresh, plugin/MCP reconciliation, workdir-skill discovery, and resulting RuntimeRegistry publication converge, and before a `TurnBinding` is returned. A reconstructed admission captures one current snapshot, compares the fingerprint before target resolution, and pins resolution, resource-policy compilation, schema, dispatch, skills, sidecar materialization, and descendants to that one matched snapshot. Equal semantics may recover across different generation ordinals; unequal semantics reject even when the ordinal is equal. There is no latest-catalog rebind.

`RuntimeSemanticFingerprintV1` is a domain-separated SHA-256 of a deterministic, length-prefixed canonical encoding with named, sorted sections covering: (1) the complete active BundleCatalog identity, canonical bundle identities, each PreparedCatalog verified digest, and visibility/reference state not already covered; (2) separate none/basic/full effective resource views, including canonical tool names, canonical input/output schema bytes or digests, dispatch/source identity, visibility, and immutable permission key/posture/policy identities; (3) every effective skill identity, precedence result, semantic metadata, and content digest, including discovered workdir skills; (4) every authoritative built-in, plugin, and MCP source identity, kind, namespace, enablement, declaration/resource/configuration digest, ordering, and collision-resolution value; and (5) reachable canonical agent/source declaration digests not already covered. All three capability views enter one fingerprint because descendants can receive different views. Generation counters, timestamps, PIDs, Arc addresses, live connections, transient health, provider permits, cancellation state, and raw secrets are excluded. Any behavior-affecting source without a deterministic declaration/resource/permission identity makes the fingerprint unavailable; omission is forbidden.

Each private versioned spawn intent for a nonterminal row that may be reconstructed after restart—including pre-`Started` `Accepted` rows because startup requeues them—atomically stores fingerprint format version, the 32-byte fingerprint, and diagnostic `ConfigGeneration`; its integrity hash covers all three. The intent also losslessly preserves the accepted post-hook SpawnMember/inline overlay, stable target, parent, background flag, batch identity/order/cardinality, and prior-start provenance, while excluding guidance, cancellation/reply channels, resolved specs/policies, Arcs, package bytes, plugin handles, and permits. An `Accepted` class lacking that fingerprint is not reconstructable and must abort on restart rather than requeue. Canonical fingerprint input bytes, RuntimeSnapshot fields, catalogs, schemas, skills, source manifests, plugin/MCP objects, credentials, and runtime handles are not persisted.

Fingerprint mismatch or unavailability, unknown format, corruption, workdir/reconciliation failure, missing or incompatible exact target/overlay, or unsupported recovered resources terminalizes through the existing private admission abort/failure surface exactly once, before target resolution where applicable and always before actor claim, Tokio task, sidecar, provider permit, or `Started`. Initial admission atomically rejects any restart-reconstructable batch whose fingerprint is unavailable. No new stable Event/wire type, catalog history, generation retention/GC, second queue/projection/runtime/catalog, or fallback to generation/latest/bundle-only/reduced-view identity is authorized.

The strict sequence remains REDs 1-2, then before launch RED 3: fingerprint determinism and section-isolation tests; unavailable-source admission/recovery tests; spawn-intent fingerprint/integrity round trip; matching-semantics/different-generation recovery; changed-semantics/same-generation rejection; dynamic workdir/plugin/MCP cases; and exact post-hook intent reconstruction. RED 8 repeats matching recovery at most once and mismatch/unavailable terminalization once. Never-started `Queued`/`Waiting` and requeued pre-`Started` `Accepted` work may reconstruct only on a fingerprint match; a `Waiting` parent that previously crossed `Started` remains stale-running work and aborts/fences without replay.

### `CONSULT-2026-08-03-ADMISSION-BINDING-FINGERPRINT-29`

```yaml
consultation_id: CONSULT-2026-08-03-ADMISSION-BINDING-FINGERPRINT-29
date: 2026-08-03
scope: app-owned semantic identity and bounded raw intent for restart-reconstructable 0.34.12 admission rows
status: Pro-arbitrated, Mac source-corrected, Sol source-validated, authoritative
choice: A
supersedes: Consult28 only where it treated RuntimeSemanticFingerprintV1 as sufficient for app-owned spawn resolution
retains: completed RuntimeSemanticFingerprintV1 sections, ConfigGeneration as diagnostic only, one RuntimeSnapshot/catalog authority, one admission journal, no derived AgentSpec persistence
```

`RuntimeSemanticFingerprintV1` remains the core-owned identity of one captured `TurnBinding` runtime view and is necessary but insufficient for cross-process spawn reconstruction. `hya-app` owns one immutable `AdmissionResolutionContext` containing the exact Harness base `AgentSpec`, `CategoryRegistry` Arc, and immutable configured-routing semantic view captured from `ProviderRouter`. Initial resolution and fingerprinting consume those same objects. Recovery captures one current runtime binding and one current app context, compares identities before target resolution, and then re-resolves only from those same captures.

The private domain-separated `AdmissionBindingFingerprintV1` embeds the versioned runtime fingerprint plus: (1) base model, exact base system-prompt bytes, and base reasoning, while excluding base name/workdir because current resolution overwrites them; (2) only category lookup and ordered model candidates actually consumed by `resolve_servable`, excluding `prompt_append` and `token_budget` while this spawn path discards them; (3) ordered configured provider identities, implementation/version, model/alias inclusion, deterministic `capabilities(model)`, route precedence, resolver-relevant non-secret configuration, and credential-slot identity/presence where construction or configured servability depends on it; and (4) an explicit `ResolverSemanticsV1` discriminator for base inheritance, category traversal, and first-capable-provider selection. Transient health, latency, quota, circuit-breaker/connection state, live permits, and credential bytes are excluded. A provider lacking deterministic configured identity makes the composite unavailable; silent omission and live probing are forbidden.

Each restart-reconstructable member row stores one canonical, uncompressed `SpawnIntentV1`: exact accepted post-hook raw member/overlay/task input, exact stable target, raw model/category choices, resident flag, parent/background, operation and batch identity/ordinal/cardinality, prior-start provenance, both fingerprint versions/values, resolver version, diagnostic generation, and integrity hash. It never stores the final/effective model, prompt, reasoning, chosen candidate, resolved `AgentSpec`, provider/catalog/resource objects, credentials, guidance, cancellation/reply state, or runtime handles.

The exact encoded per-row maximum is 1,048,576 bytes inclusive of all intent tags and integrity metadata but exclusive of fixed scheduler columns. Exactly that size is accepted; 1,048,577 bytes rejects typed. Every member is canonically encoded and size-checked before the admission transaction; one oversized or unidentifiable member rejects the complete batch with zero rows or allocations. There is no truncation, compression, chunking, auxiliary payload table, or reconstruction from stable Events.

Unknown versions, corrupt/noncanonical encodings, either fingerprint mismatch, unavailable configured identity, or authoritative-context capture failure causes atomic initial rejection or exactly-once recovery abort before target resolution, actor claim, task, sidecar, provider permit, or `Started`. There is no ConfigGeneration/BundleRegistry fallback, latest-definition rebind, reduced identity, transient-state selection, or persisted derived-launch fallback. A matched configured provider that is transiently unavailable fails through ordinary execution and does not select another candidate.

Before the former launch RED 3, the strict order adds: composite determinism across restart/order/publication/generation/transient-health changes; behavior-only base isolation; ordered category selection with unused-field non-sensitivity; configured-provider identity and unavailable-provider fail-closed tests; raw intent round trip/integrity with explicit absence of derived launch fields; and exact 1 MiB boundary plus all-or-none oversized-batch rollback. RED 3 then requires both fingerprints; RED 8 proves matching launch at most once and stable mismatch/unavailable terminalization without later rebind.

### `CONSULT-2026-08-04-QUEUED-CALLER-REPLY-30`

```yaml
consultation_id: CONSULT-2026-08-04-QUEUED-CALLER-REPLY-30
date: 2026-08-04
scope: 0.34.12 live queued caller-reply semantics only
status: Pro-arbitrated, Mac source-corrected, Sol source-validated, authoritative
choice: A
supersedes: any implication that durable Queued admission acknowledges a caller or may allocate a placeholder child
retains: one admission_journal authority, exact 100/156/256 capacity, deterministic promotion, existing public running/final outcomes, stable Event/DTO/wire bytes
```

A durable `Queued` commit is internal admission only. It neither completes `SpawnRequest.reply` nor creates a caller-visible acknowledgement. Foreground keeps the sole process-local sender while any admitted member is nonterminal; promotion, `Accepted`, `Started`, and partial completion are insufficient. It completes exactly once with the existing final result only after every member in that request is durably terminal. If a mixed batch is cancelled, queued rows terminalize without allocation and accepted/started rows finish their existing cancellation/finalization paths before the request completes.

A one-member background transient or initial resident request also keeps its sender while queued. Only after its row is promoted `Queued -> Accepted`, crosses the exact member-specific durable `Started` compare-and-set, and successfully registers the real child session or resident handle may it send the existing `MemberOutcome { status: "running", session: <real session> }` or real resident handle, consume the sender, and detach. Promotion or `Started` alone cannot reply. A post-promotion failure/cancellation before real registration sends no fabricated running outcome.

Queued cancellation and promotion linearize through the existing serialized `admission_journal` SQLite writer transaction/current-state predicate on the same row. Cancellation wins only when `Queued -> Cancelled` commits first; later promotion observes non-Queued and returns no launch, with zero task/session/actor/sidecar/provider/`Started` allocation. Promotion wins only when `Queued -> Accepted` commits first; the queued-cancel path then loses and existing post-promotion cancellation applies. For a queued-only background/resident request, a cancellation winner commits durable `Cancelled` first and takes/sends one `Err(SpawnError::Cancelled)`. Foreground does not reply until its whole admitted batch is terminal.

The sender lives only as an `Option` in the operation's existing process-local request/launch owner and is taken on the one winning completion path. A dropped receiver makes send fail locally; the owner ignores that failure and continues durable convergence without panic, implicit cancellation, retry, or a second registry. No sender survives restart, and restart/caller disconnect creates no durable reply replay or reconnect promise.

The Mac sync-area source used for the first channel-shape check was stale. Exact fuji1 HEAD `dd2cade149df57573938f89e19c322d2f76c189e` already has `SpawnRequest.reply: oneshot::Sender<Result<Vec<MemberOutcome>, SpawnError>>`; `SpawnerPlane::spawn_inner` already flattens a missing oneshot channel to `SpawnError::Unavailable`; and `SpawnError` already contains its existing typed variants. This factual correction does not change Option A. Do not migrate the reply type or flattening. Add only unit `SpawnError::Cancelled` with display `spawn cancelled before activation`; reserve `Unavailable` for its existing unavailable/channel-loss cases.
