...
This matrix is anchored to committed `fuji1` HEAD
`267bfc3c6c66e46fe8514e2e70657489f853b7f0`. The active, uncommitted
`0.34.3` admission slice changes admission timing, spawn transport capacity,
and typed overload propagation; it does not change the agent-definition fields
inventoried here.

The isolated branch now rests on fetched `origin/main`
`156d0ad3c50aea67dfac0054485eb6991e77308b`. Its only delta from the matrix
anchor is a README icon-reference change, so this field inventory remains
source-current.
...
The MacBook Air coordinator returned Pro round six on 2026-07-31. The
controlling plan uses deterministic build-time preparation, authoritative
embedded package bytes plus a digest-bound index, and immutable built-in origin
in `0.34.8`. Startup does not call the package installer. Nothing in this file
changes the active `0.34.3` scope.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Built-in catalog membership: `build`, `plan`, `general`, `explore`, `compaction`, `title`, `summary` | `compat::agent_catalog::NATIVE_AGENTS`, `native_entries`; `compat::agent_native_prompts::get` | Not parsed; compiled into the binary | Entry selected by name and overlaid onto the server base spec | Four built-ins have exact embedded prompt text; others inherit the server base prompt | `mode`/`hidden` determine list placement | All names are resolvable today; see unknown fallback row | All currently share the Harness registry and runtime permission plane | Public names enter session/member/roster events | One authoritative build-time-prepared embedded native built-in Bundle; retain these public IDs explicitly and mark `origin=builtin, immutable=true` | Freeze catalog ordering/metadata, exact prompt bytes, tool schemas, and event traces before cutover; compare native Bundle output byte-for-byte; boot without install | A_EFFECTIVE | SOURCE-CONFIRMED; GA-BLOCKING; `0.34.8` BUILD-TIME-EMBEDDED |
...
| `mode` / primary-mode directories | `InlineAgent::mode`; `AgentFrontmatter::mode`; `append_inline_agents(primary)`; `disk_agent(primary)`; `agent_defaults::selectable`; TUI `createAgent` | Accepts strings; `.opencode/mode(s)` forces `"primary"`; new entry defaults to `"all"` | Not represented | No request effect | Exactly `"subagent"` is excluded from main-agent picker/default selection; `hidden` is a separate filter | Resolver does not prohibit spawning `"primary"`/`"all"` agents | No policy effect | Agent ID, not mode, is persisted | `role: main|subagent` carries selection class only. Map current `"subagent"` to `subagent`; map other effective values to `main`. Do not encode root-session lifetime in `role` | Differential TUI/default-selection/list-agents test for every current mode, plus spawn test proving role does not silently change reachability | A_EFFECTIVE | SOURCE-CONFIRMED; GA-BLOCKING |
| `hidden` | `InlineAgent::hidden`; `AgentFrontmatter::hidden`; `AgentEntry::hidden`; `agent_definitions`; TUI `createAgent` | Accepted; default false | Not represented | No request effect | Hidden entries are omitted from both main picker and model-facing `list_agents` | Direct name resolution still finds a hidden entry | No policy effect | ID can still exist in historical events | Native Bundle IR must represent hidden independently from `role`; exact manifest spelling awaits schema finalization | Catalog/API/TUI/list-agents golden covering hidden main and hidden subagent | A_EFFECTIVE | SOURCE-CONFIRMED; GA-BLOCKING |
...
| Unknown `subagent_type` fallback | `subagent_resolve::resolve_subagent`; test `unknown_type_keeps_name_and_falls_back` | Any requested string is accepted by task input | Keeps requested `AgentSpec.name` but takes native `general` prompt/reasoning/model fallback | Sends fallback content under the unknown agent name | Unknown name is not catalog-listed | Explicit source-tested fallback to `general` | Task permission is checked against the unknown requested string | Events persist the unknown requested ID | Conflicts with fail-closed missing-ID resolution. Do not choose silently | Characterize exact prompt/model/event behavior; explicit owner ruling must choose parity or a breaking fail-closed change before native cutover | A_EFFECTIVE with architecture conflict | SOURCE-CONFIRMED; `0.34.8` OWNER-BLOCKED |
| Main-agent spawn reachability | `TaskTool::execute`; `subagent_resolve::resolve_subagent` | Task accepts any `subagent_type`; resolver does not filter catalog `mode` | A `main`/`primary` entry can become a child `AgentSpec` | Uses that entry's resolved prompt/model | Role only affects picker | Current behavior allows spawning main entries; permission plane may deny by requested name | Existing Task permission applies; no `can_spawn` catalog policy | Normal member/session events | Native built-in bundle `can_spawn` graph must preserve current effective reachability unless the owner approves a breaking restriction; new installed bundles may be narrower | Spawn every built-in role through fake provider; record allow/deny and event outcomes | A_EFFECTIVE | SOURCE-CONFIRMED; GA-BLOCKING |
| Full Harness resource view for current agents | `engine::turn::messages::filtered_tool_schemas`; `SessionEngine` planes; runtime registry wiring | No per-agent resource parser is effective | `AgentSpec` carries no resource view | Every current agent sees the same ordered registry schemas, except model-specific `apply_patch` versus edit/write filter | TUI MCP toggles global manager state | Child receives the same engine registry/planes | Existing `PermissionPlane` authorizes calls; skill/MCP/builtin/plugin tools share registry/planes | Tool call/result/error events use existing stable names | Built-in native agents request `resource_view: full`; effective view remains Harness-policy-narrowed and must reproduce schema order and callable set | Byte-for-byte schema list plus successful/denied builtin, skill, MCP, and plugin calls for main/transient/resident | A_EFFECTIVE | SOURCE-CONFIRMED; GA-BLOCKING |
...
| `general` | subagent | visible to model catalog, hidden from main picker | falls back to base prompt | Preserve spawnability and current fallback role pending the unknown-ID ruling |
| `explore` | subagent | visible to model catalog, hidden from main picker | embedded `explore.txt`, trimmed at end | Freeze exact bytes |
| `compaction` | main-class metadata | hidden | embedded `compaction.txt`, trimmed at end | Preserve internal call sites and public historical ID |
| `title` | main-class metadata | hidden | embedded `title.txt`, trimmed at end | Preserve internal call sites and public historical ID |
| `summary` | main-class metadata | hidden | embedded `summary.txt`, trimmed at end | Preserve internal call sites and public historical ID |
...
- **Unknown-new-spawn behavior remains open:** the round-six material returned
  to the `fuji1 remote worker` did not include an owner ruling on the
  source-confirmed fallback to `general`. The exact `0.34.8` resolver/cutover
  dependency stays blocked; `0.34.3`–`0.34.7` do not depend on it.
