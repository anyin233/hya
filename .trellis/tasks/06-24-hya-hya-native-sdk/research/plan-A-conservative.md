...
**Wire `hya` ↔ `hya` via a native in-process Rust SDK (no HTTP).**

Framing: smallest correct change. Re-use the `Transport` seam, the existing
`hya_server::router(state)`, the existing `/global/event` projection, and the
`tower::oneshot` + `http_body_util::BodyExt` patterns already used in 30+ tests
(`crates/hya-server/tests/api.rs:53`, `opencode_event_api.rs:105`,
...
**Bottom line.** Add (1) one new tiny crate `hya-bootstrap` (move the bootstrap
glue out of the `hya-cli` binary into a library — nothing else), (2) one new
feature-gated module `hya_sdk::hya_native` (~150 LOC: `HyaNativeTransport`
over `tower::oneshot`, plus `spawn_hya_event_bridge` that oneshot-fetches
...
- **No re-projection of the bus.** The 796-line `/global/event` projection in
  `crates/hya-server/src/opencode/event.rs` already produces exactly the
...
- **No new `hya-hya` crate.** A feature on `hya-sdk` is one Cargo.toml change vs.
...
- **No moving `auth`, `agent_cmd`, `models_cmd`, `tui*`, `rpc`, `cli_args`** out
  of `hya-cli`. Only the bootstrap functions the engine actually needs.
...
```
crates/
├── hya-bootstrap/                 # NEW, tiny (~600 LOC moved verbatim)
│   ├── Cargo.toml
│   └── src/
...
│       │                           # move of hya-cli/src/skills.rs)
│       ├── config.rs               # verbatim move of hya-cli/src/config.rs
│       ├── plugins.rs              # verbatim move of hya-cli/src/plugins.rs
│       ├── formatter_config.rs    # verbatim move of hya-cli/src/formatter_config.rs
│       ├── permission.rs           # verbatim move of hya-cli/src/permission.rs
│       └── team.rs                 # spawn_team_supervisor (moved from main.rs)
│
├── hya-sdk/                        # ONE new module + ONE Cargo feature
│   ├── Cargo.toml                  # add `native-hya` feature (opt-in)
│   └── src/
│       ├── lib.rs                  # add `#[cfg(feature="native-hya")] pub mod hya_native;`
│       └── hya_native.rs          # NEW (~150 LOC): HyaNativeTransport
│                                   # + spawn_hya_event_bridge
│                                   # + HyaNativeClient (= ApiClient<HyaNativeTransport>)
│
├── hya/                            # binary wiring only
│   ├── Cargo.toml                  # enable hya-sdk/native-hya
│   └── src/main.rs                 # add --native flag + Transport::Hya variant
│
├── hya-cli/                       # use hya-bootstrap; delete duplicates
│   ├── Cargo.toml                  # +hya-bootstrap dep
│   └── src/
│       ├── main.rs                 # `use hya_bootstrap::*;` — remove moved fns
│       ├── serve.rs                # `use hya_bootstrap::*;` — unchanged otherwise
...
- **Why a new crate, not a `hya_server::bootstrap` module?** `hya-server`
  currently depends on `hya-core`, `hya-mcp`, `hya-proto`, `hya-store`,
  `hya-tool` only (see `crates/hya-server/Cargo.toml`). Bootstrap needs
  `hya-provider` and `hya-plugin`. Folding bootstrap into `hya-server` would
  expand `hya-server`'s role from "HTTP routing over engine" to "router + config
  + provider loader + plugin host", and force every downstream consumer of
  `hya-server` (and there will be more as the project grows) to drag in
  provider/plugin code they don't need. A focused 600-LOC `hya-bootstrap` crate
  is the smaller intrusion.
- **Why feature-gate `hya_native` instead of always-on?** A `hya-sdk` consumer
  that only needs the HTTP transport shouldn't be forced to compile
  `hya-server` + `hya-bootstrap` + `axum` + `http-body-util`. Gating also
...
```
hya-proto, hya-provider, hya-tool, hya-store, hya-mcp, hya-plugin
   └── hya-core
          └── hya-bootstrap (NEW)
                 ├── hya-cli         (was direct: hya-cli → all-of-above)
                 └── hya-sdk[native-hya]  ──┐
                       └── hya-tui          │
                              └── hya       │
hya-server ── hya-cli                     │
...
`hya-cli` ALREADY depends on `hya-server` (the `serve` command); adding
`hya-bootstrap` adds no cycle. `hya-sdk[native-hya]` adds a new edge into
`hya-server` + `hya-bootstrap` — both leaves, no cycle.
...
| from `crates/hya-cli/src/…`                | to `crates/hya-bootstrap/src/…` |
|---------------------------------------------|----------------------------------|
...
### 2.3 Public surface of `hya-bootstrap`

```rust
// crates/hya-bootstrap/src/lib.rs
pub mod agent;
pub mod config;
...
```rust
// crates/hya-bootstrap/src/runtime.rs (key types)

/// Identical to today's `RuntimeConfig` in hya-cli/src/main.rs:322.
pub struct RuntimeConfig {
    pub router: hya_provider::ProviderRouter,
    pub model: String,
    pub models: Vec<crate::config::ModelEntry>,
    pub mcp: std::collections::BTreeMap<String, hya_mcp::McpServerConfig>,
    pub plugins: Vec<hya_plugin::config::PluginSpec>,
    pub default_agent: Option<String>,
}

/// Everything `hya` (or `hya-cli serve`) needs to assemble an `AppState`,
...
pub struct EngineRuntime {
    ...
    pub asks: tokio::sync::mpsc::UnboundedReceiver<hya_tool::AskRequest>,
    pub questions: tokio::sync::mpsc::UnboundedReceiver<hya_tool::QuestionRequest>,
    pub mcp_manager: hya_mcp::McpManager,
    pub plugin_host: std::sync::Arc<hya_plugin::PluginHost>,
    pub default_agent: Option<String>,
    pub model: String,
    pub models: Vec<crate::config::ModelEntry>,
}
...
Notice: NO `axum`, NO `hya-server`, NO `reqwest`, NO `clap`. Bootstrap is pure
engine construction.

### 2.5 Hya-cli call-site changes

Two short diffs (a few lines each):

```rust
// crates/hya-cli/src/main.rs — top of file
use hya_bootstrap::{
...
use hya_bootstrap::permission::{PermissionPolicy, spawn_auto_responder};
...
```rust
// crates/hya-cli/src/serve.rs
use hya_bootstrap::{agent_with_model, build_engine, host_info, open_store,
...
deps (and only when `native-hya` is enabled) is `hya-server` +
`hya-bootstrap`; neither depends on `hya-sdk`. `hya-cli` already depends on
both `hya-server` and (after this change) `hya-bootstrap`. No cycles.

---

## 3. `HyaNativeTransport` design

File: `crates/hya-sdk/src/hya_native.rs` (NEW, ~120 LOC).
...
native-hya = ["dep:hya-server", "dep:hya-bootstrap", "dep:tower",
...
# new, optional:
hya-server = { workspace = true, optional = true }
hya-bootstrap = { workspace = true, optional = true }
...
```toml
[dependencies]
hya-sdk = { path = "../hya-sdk", features = ["native-hya"] }
hya-tui = { path = "../hya-tui" }
reqwest = { ... }                  # keep for now (HTTP path still works)
tokio = { ... }
hya-bootstrap = { workspace = true }  # NEW — main.rs constructs EngineRuntime
hya-server = { workspace = true }     # NEW — main.rs calls server::router(state)
```

### 3.2 The transport (exact code shape)

```rust
//! crates/hya-sdk/src/hya_native.rs
//! In-process Transport that calls `hya_server::router(state)` via `tower::oneshot`.
//! No TCP. No reqwest. The router is `Clone` (axum::Router wraps an Arc);
//! every request clones-and-oneshots, exactly like crates/hya-server/tests/api.rs.
...
const BASE_URL: &str = "http://hya.internal";
...
| Knob | Decision | Why |
|---|---|---|
| `base_url` | constant `"http://hya.internal"` | Same trick `NativeTransport` (native.rs:273) already uses. It's never network-resolved; it just keeps `Client::base_url()` non-empty for log/breadcrumb sites. |
...
| Couples hya to | the SSE wire shape (already a frozen contract) | hya's internal `Envelope`/`Event` enum |
...
```rust
//! still in crates/hya-sdk/src/hya_native.rs
pub fn spawn_hya_event_bridge(
    router: axum::Router,
    directory: String,
...
- **Shutdown.** The `tokio::task::JoinHandle` returned is stored on
  `Transport::Hya`; `.shutdown()` aborts it (mirrors current
...
```rust
native: bool,        // --native: in-process hya, no HTTP
```
...
```
  --native           run hya in-process (no HTTP, no spawn)
```
...
enum Transport {
    ...
    Hya {                                       // NEW
        plugin_host: Arc<hya_plugin::PluginHost>,  // explicit lifecycle owner
...
async fn connect(
    ...
    if args.native && args.server.is_none() {
        ...
        let rt = hya_bootstrap::build_engine(None, "").await?;
        let plugin_host = rt.plugin_host.clone();
        let workspace_adapters = rt.plugin_host.workspace_adapters();
        let state = hya_server::AppState::new(rt.engine, rt.agent)
            .with_question_requests(rt.questions)
            .with_mcp_manager(rt.mcp_manager)
        ...
        let event_task = hya_sdk::hya_native::spawn_hya_event_bridge(
            router.clone(),
            directory.to_owned(),
            event_tx,
        );
        ...
        let client: Arc<dyn Client> = Arc::new(
            hya_sdk::ApiClient::with_transport(
                hya_sdk::hya_native::HyaNativeTransport::new(router, directory),
            )
        );
        return Ok((client, Transport::Hya { plugin_host, event_task }));
    }
...
fn shutdown(self) {
    match self {
        ...
        Transport::Hya { plugin_host, event_task } => {
            event_task.abort();
            // McpManager was moved into AppState (held by the router, held by
            // HyaNativeTransport, held by the Arc<dyn Client>) — dropping the
...
`HyaNativeTransport`, which holds the router, which holds the AppState. When
the TUI exits, `spawn_connect`'s join handle returns the `Transport::Hya`
...
`crates/hya-cli/src/serve.rs:108` pattern that keeps the host alive until
graceful shutdown.
...
|---|---|
| `--native` (no `--server`) | new Hya branch |
| `--native --server <url>` | reject in `Args::parse` (`InvalidInput`) |
| `--http` / `--opencode` / `--server` / default (no `--native`) | UNCHANGED from today |
| `--yolo` (future) | inside Hya branch, swap `with_permission_requests` for `spawn_auto_responder(asks, PermissionPolicy::Yolo)` — copy the 4-line pattern from `serve.rs:88-93`. Out of scope for this PR. |
...
File: `crates/hya-sdk/tests/zero_http_hya_native.rs` (NEW).

```rust
#![cfg(feature = "native-hya")]

use http_body_util::BodyExt;
use hya_sdk::{ApiClient, Client, hya_native::HyaNativeTransport};
use std::sync::Arc;
use tokio::sync::mpsc;
use hya_bootstrap::build_engine;
use hya_server::{AppState, router as build_router};
...
async fn full_turn_no_tcp_sockets_created() {
    ...
    let client: Arc<dyn Client> = Arc::new(
        ApiClient::with_transport(HyaNativeTransport::new(router.clone(), "."))
    );
    ...
    let bridge = hya_sdk::hya_native::spawn_hya_event_bridge(
...
A `crates/hya-bootstrap/tests/no_reqwest_in_native_path.rs` test (compile-only)
that does `let _: hya_bootstrap::EngineRuntime;`. hya-bootstrap has no
reqwest dep (verified in §2.4). If reqwest ever sneaks into the bootstrap deps,
`cargo tree -p hya-bootstrap | grep reqwest` in CI catches it. Add this as a
1-line CI step.
...
`crates/hya-sdk/tests/hya_native_transport.rs` (NEW; `#![cfg(feature="native-hya")]`):
...
`crates/hya-sdk/tests/zero_http_hya_native.rs` (verbatim §6.1).

### 7.3 Existing tests

The `crates/hya-server/tests/api.rs` and 70+ opencode_*_api.rs tests already
...
`.trellis/tasks/06-24-hya-hya-native-sdk/qa/native-tmux.md`):
...
|---|---|---|---|---|
| 1 | **`hya-bootstrap` extraction misses a private dep** (e.g., `hya-cli::tui::history` reaches into `config::ModelEntry`). Build breaks. | Medium | Low | The `ModelEntry`, `RuntimeConfig` types are already `pub` (or used as field types of `tui::RunOptions`). Run `cargo build --workspace` after each move file; the borrow checker forces every miss to surface. Atomic commit per file. |
...
| 8 | **Cargo feature interaction with `cargo build --workspace`** — `hya` enables `hya-sdk/native-hya`, dragging `hya-server` into the build for everyone. | Medium | Low | Acceptable — `hya-server` is already a workspace member and builds anyway. The feature exists for downstream crates, not the workspace build. |
| 9 | **Periodic merge with origin/main brings new routes/events that need projection updates.** | Medium | Medium | Zero new exposure on the native path — the projection is server-side, so any rebase automatically picks up new routes/events. The contract is wholly inside `hya-server::opencode`. |
...
`crates/hya-bootstrap/**` (new) and `crates/hya-cli/{Cargo.toml,
src/main.rs, src/serve.rs}`.
- Create `crates/hya-bootstrap/Cargo.toml` per §2.4.
- Move files per §2.1, verbatim (`git mv` where possible — keeps blame).
- Write `crates/hya-bootstrap/src/lib.rs` per §2.3.
- In `crates/hya-bootstrap/src/runtime.rs`, expose `EngineRuntime` and
  `build_engine(model_override, db)` that orchestrates `open_store + resolve_runtime
  + build_session_engine`.
- Update hya-cli per §2.5; delete the moved modules from `hya-cli/src/`.
- Add `hya-bootstrap = { path = "crates/hya-bootstrap" }` to the workspace
  `[workspace.dependencies]` table.
- Gate: `cargo build --workspace` green; `cargo test -p hya-cli` green;
  `cargo test -p hya-server` green.

### Parallel after A1 (run as four concurrent subagents)

**A2 — `HyaNativeTransport` only.** Files touched: only
`crates/hya-sdk/Cargo.toml` (add feature + optional deps) and the FIRST half of
`crates/hya-sdk/src/hya_native.rs` (just the transport, NOT the event bridge).
Update `crates/hya-sdk/src/lib.rs` with `#[cfg(feature="native-hya")] pub mod
hya_native; #[cfg(feature="native-hya")] pub use hya_native::{HyaNativeTransport,
HyaNativeClient};`.
- Gate: `cargo build -p hya-sdk --features native-hya` green.

**A3 — SSE event bridge.** Files touched: SECOND half of
`crates/hya-sdk/src/hya_native.rs` only (`spawn_hya_event_bridge` function).
...
`crates/hya-sdk/tests/hya_native_transport.rs` ONLY (NEW file). No code
overlap with A2/A3.
- Gate: `cargo test -p hya-sdk --features native-hya` green for the new tests.

**A5 — accept-gate integration test.** Files touched:
`crates/hya-sdk/tests/zero_http_hya_native.rs` ONLY (NEW file). Uses the
public surface introduced by A2/A3 — so A5 must merge AFTER A2 and A3.
- Gate: `cargo test -p hya-sdk --features native-hya --test
  zero_http_hya_native` green.

### Sequential tail (one subagent each, in this order)

**A6 — hya/main.rs wiring.** Files touched: `crates/hya/Cargo.toml` (add
`hya-bootstrap`, `hya-server`, `hya-sdk` feature), `crates/hya/src/main.rs`
(add `--native` flag, `Transport::Hya` variant, branch in
...
- `.trellis/tasks/06-24-hya-hya-native-sdk/qa/native-tmux.md` (NEW).
...
- **`with_permission_requests` vs `spawn_auto_responder`** — the AppState
  ...
  an empty channel.
- **`hya-bootstrap` MUST NOT depend on `axum` or `hya-server`.** Bootstrap is
  pure engine construction. If a future change needs server-aware bootstrap, it
  belongs in a different crate (or in `hya-server::bootstrap` if the policy
  flips).
...
1. **Feature-gate the HTTP path** (`HttpTransport`, `events.rs`, `server.rs`)
   ...
   --no-default-features --features hya-sdk/native-hya` produce a binary with
...
2. **Flip `--native` from opt-in to default.** Replace the current default
   (spawn `hya serve` over HTTP) with the in-process Hya path; relegate the
