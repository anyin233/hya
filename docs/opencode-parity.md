...
| Event stream | `/api/event` and `/global/event` send OpenCode-shaped SSE with `server.connected` and yaca envelope payloads. Missing OpenCode's full native event type taxonomy, durable event replay, and location-aware filtering for every event source. |
