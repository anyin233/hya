...
session context, session title update/delete, legacy/v2 command and shell routes,
v2 prompt admission, active provider/model compatibility, permission/question
queue, project copy, project metadata/init/update, MCP status, and v2
filesystem compatibility, plus legacy session todo compatibility increments.
...
| OpenCode session API | Partial | yaca accepts prefixed `ses_...` and `msg_...` IDs and returns OpenCode-style session info, children, todo lists, `[{ info, parts }]` message lists, single-message reads, `limit`/`before` pagination headers, legacy and v2 title update/delete plus command/shell routes, v2 `/api/session` list/create/get/context wrappers, v2 prompt admission, v2 `POST /api/session/:sessionID/agent` plus `/model` switching endpoints, and v2 compact/wait routes with current unavailable semantics. |
