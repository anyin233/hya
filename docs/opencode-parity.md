...
| OpenCode instance metadata API | Partial | yaca exposes legacy `/path`, `/agent`, `/command`, `/skill`, `/lsp`, `/formatter`, `/instance/dispose`, `/vcs`, `/vcs/status`, `/vcs/diff`, `/vcs/diff/raw`, `/vcs/apply`, plus v2 `/api/location`, `/api/agent`, `/api/command`, and `/api/skill`. LSP/formatter are empty status arrays for now. |
