...
| OpenCode session API | Partial | yaca accepts prefixed `ses_...` and `msg_...` IDs and returns OpenCode-style session info, children, todo lists, `[{ info, parts }]` message lists, single-message reads, `limit`/`before` pagination headers, legacy and v2 title/metadata/permission/archive update/delete/init plus command/shell routes, legacy permission respond, message deletion, part deletion, v2 `/api/session` list/create/get/context wrappers, v2 prompt admission, v2 `POST /api/session/:sessionID/agent` plus `/model` switching endpoints, and v2 compact/wait routes with current unavailable semantics. |
