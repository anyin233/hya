...
`/log` and `/auth/:providerID`, legacy session todo compatibility, and queued
TUI HTTP route increments, and basic provider auth list/logout CLI support.
...
| OpenCode instance metadata API | Partial | yaca exposes legacy `/path`, `/agent`, `/command`, `/skill`, `/lsp`, `/formatter`, `/instance/dispose`, `/vcs`, `/vcs/status`, `/vcs/diff`, `/vcs/diff/raw`, `/vcs/apply`, OpenCode `/log` plus `/auth/:providerID` token mutation, v2 `/api/health`, `/api/location`, `/api/agent`, `/api/command`, `/api/skill`, `/api/event`, `/api/reference`, read-only `/api/integration` discovery, and OpenCode TUI publish/direct/control queue routes. LSP/formatter are empty status arrays for now. |
...
| Provider/auth breadth | OpenCode `/auth/:providerID` accepts `api`, `oauth`, and `wellknown` payloads and persists the effective token to yaca's local token file; CLI supports `login`, `auth list`, and `auth logout`. The already constructed provider router is not hot-reloaded after token mutation. Missing AI SDK provider breadth, full models.dev metadata/autoload, provider status/cost/limit metadata, OAuth authorize/callback flows, provider auth methods, credential mutation, integration connect/auth flows, and Console/org switching. |
