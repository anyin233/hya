...
yaca-core::SessionEngine
      ...
      +--> yaca-tool executes builtin tools behind PermissionPlane
...
| `yaca-core` | [`../crates/yaca-core/src/lib.rs`](../crates/yaca-core/src/lib.rs) | Session engine, event bus, turn loop, goal/loop drivers, team primitives, worktree helpers. |
| `yaca-server` | [`../crates/yaca-server/src/lib.rs`](../crates/yaca-server/src/lib.rs) | Axum HTTP and SSE API over `SessionEngine`. |
...
| `yaca-cli` | [`../crates/yaca-cli/src/main.rs`](../crates/yaca-cli/src/main.rs) | Umbrella binary: TUI, `exec`, goal mode, server, tail-session, config loading. |
...
| [`tool.rs`](../crates/yaca-tool/src/tool.rs) | Tool trait, registry, builtin implementations, output truncation. |

Builtins currently include:

| Tool | Permission action | Behavior |
| --- | --- | --- |
| `read` | `Read` | Read a file and return truncated content. |
| `write` | `Edit` | Create parent directories and write file content. |
| `edit` | `Edit` | Replace the first occurrence of a string in a file. |
| `glob` | `Glob` | Walk the working directory and match a simple `*` pattern. |
| `grep` | `Grep` | Search for substring matches under a path. |
| `shell` | `Bash` | Run `sh -c` in the agent workdir. |

Tool output is capped at 16 KiB for large text fields, and list-style tools cap
returned entries at 500 while preserving a `total` count.
...
| Module | Purpose |
| --- | --- |
| [`engine.rs`](../crates/yaca-core/src/engine.rs) | `SessionEngine`, session creation, prompt admission, assistant turn loop, tool execution. |
...
`yaca-server` exposes the engine over HTTP:
...
| [`../crates/yaca-server/tests`](../crates/yaca-server/tests) | API create/prompt/replay and bad session handling. |
...
yaca-proto
  ^  ^  ^  ^  ^
  |  |  |  |  |
provider tool store server/client/tui
        \   \     /
          yaca-core
              ^
              |
