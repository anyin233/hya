...
| T2.32 | [p37_project_permission_scope.rs](../../crates/hya-e2e/tests/p37_project_permission_scope.rs) | manual mode: a session reads any root of its multi-root Project without asking; a read outside every root asks `ExternalDirectory` for the concrete `<dir>/*` (never a global `*`); answering allow-always then lets a second session of the *same* Project read a sibling file in that directory without asking, but a session of a *different* Project asks again (ADR-0026) | e2e (Track P) |
| T2.33 | [p37_project_permission_scope.rs](../../crates/hya-e2e/tests/p37_project_permission_scope.rs) | `yolo` mode auto-approves a read outside every Project root without ever raising an `ExternalDirectory` ask | e2e (Track P) |
| T2.34 | [p37_project_permission_scope.rs](../../crates/hya-e2e/tests/p37_project_permission_scope.rs) | `bash`'s `cwd` outside every Project root never raises `ExternalDirectory`, even in manual mode where the command itself still asks `Action::Bash` | e2e (Track P) |
| T2.35 | [p38_relay.rs](../../crates/hya-e2e/tests/p38_relay.rs) | secure relay golden path, pinned `t=grpc`: real `hya proxy`, `hya serve --relay` (FakeLlm), and `hya bridge - --json --exit-with-stdin` (link on stdin), every client request through the bridge URL only: health and bootstrap; a two-root Project created remotely and a session in it without a workdir (lands in root #1); one turn reads a file in root #2 without asking and one outside every root through an `ExternalDirectory` ask answered over the bridge (both results reach the model and the transcript); the session SSE stream shows the ask and the turn finishing; a PTY created, token-connected, and WebSocket-upgraded over the bridge echoes a marker; a temporary session; `GET /v1/relay/link` is `permission_denied` over the bridge but works on loopback; `hya serve relay rotate` makes the running bridge answer `503 unavailable` "rejected the relay link" while a new bridge with the new link works; SIGTERM delivers `serverStopping {reason: signal}` to a bridge-side SSE client, then the bridge answers `503 remote backend is offline`. A transparent TCP capture hop in front of the proxy (both `--relay` and the link point at it) records every byte: it sees the room id (control) but never the prompt, file contents, answer, PTY output, or either link's PSK (raw, base64url, fragment) | e2e (Track P) |
| T2.36 | [p38_relay.rs](../../crates/hya-e2e/tests/p38_relay.rs) | T2.35 over the pinned WebSocket binding (`t=ws`), capture assertions included | e2e (Track P) |
| T2.37 | [p38_relay.rs](../../crates/hya-e2e/tests/p38_relay.rs) | TLS to the proxy (`--tls-cert/--tls-key` with an rcgen self-signed certificate; `--relay-ca` on `serve` and `bridge`; `hya://` link; `t=auto`): health, a session, and a prompt through the bridge; the capture hop sees no prompt, answer, or PSK bytes | e2e (Track P) |
