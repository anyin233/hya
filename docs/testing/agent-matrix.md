...
| T2.29 | [p34_bundle_views.rs](../../crates/hya-e2e/tests/p34_bundle_views.rs) | a packaged Plugin with a `kind: bun` `extensions.process` (a python3 fixture) declares view `usage` and tool `usage_probe`; the tool call receives a capability (`context.describe` + `session.usage`); after a turn that spawns a `general` subagent with FakeLlm reporting deterministic usage, `GET /v1/sessions/{session}/views/acme%2Fusage-views/usage` returns one row per session of the tree plus a merged total whose rounds equal the FakeLlm calls and whose per-model input/output/thinking split follow the reported usage; `?scope=session` narrows to one row; unknown view/bundle/session answer `view_not_found`/`session_not_found`; `GET /v1/sessions/{session}/views` lists the view | e2e (Track P) |
| T2.30 | [p35_token_summary.rs](../../crates/hya-e2e/tests/p35_token_summary.rs) | packaged `hya-extra/token-summary` Bun process bundle; after a turn that spawns a subagent via `task` with FakeLlm reporting deterministic (known-split) usage, `GET /v1/sessions/{session}/views/hya-extra%2Ftoken-summary/usage` returns per-model `input`/`cache_creation`/`cache_read`/`output`/`thinking`/`visible_output`/`rounds` matching the reported usage across both sessions of the tree, with the grand total equal to the one model used; `?scope=session` narrows to the root's own rounds; a second turn where `build` calls tool `token-summary__token_summary` and the follow-up model request carries the rendered Markdown table | e2e (Track P) |
