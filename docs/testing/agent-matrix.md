...
| T2.31 | [p36_wait_archive.rs](../../crates/hya-e2e/tests/p36_wait_archive.rs) | the lead spawns `hya-worker` and calls `wait`: the worker's `report` wakes the wait inside the lead's own turn (`Subagents finished.` plus the report reach the lead's follow-up request); a second run `wait`s for the idle worker, `archive`s it (`Archived \`main/hya-worker-1\``), and `send` to its handle wakes it again (the worker's next request carries the mail) | e2e (Track P) |
