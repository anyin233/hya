# hya, Pi, and OpenCode Feature Comparison

Last researched: 2026-06-30.

This page compares hya with upstream stock Pi (`earendil-works/pi`) and current
OpenCode (`anomalyco/opencode` plus `opencode.ai`). It intentionally does not
use the Oh My Pi harness/fork as the Pi baseline, and it does not use the
archived `opencode-ai/opencode` repository as the OpenCode baseline.

## Executive Summary

| Area | hya | Pi coding agent | OpenCode |
| --- | --- | --- | --- |
...
- **hya baseline:** this repository's current architecture and docs, especially
  ...
  OpenCode compatibility tracker [OpenCode Parity Matrix](opencode-parity.md).
...
- **OpenCode baseline:** current OpenCode docs at
  [opencode.ai/docs](https://opencode.ai/docs/) and the active
  [`anomalyco/opencode`](https://github.com/anomalyco/opencode) TypeScript
  codebase. The older [`opencode-ai/opencode`](https://github.com/opencode-ai/opencode)
  repository is archived and says the project continued as Crush; it is not used
  as the current feature baseline.
- **Known drift:** OpenCode docs list a built-in `scout` subagent, but the
  reviewed `packages/opencode/src/agent/agent.ts` source confirmed `build`,
...
OpenCode-shaped HTTP MCP routes report status and accept compatibility add or
...
content on demand. The OpenCode-compatible server also scans OpenCode-style
...
[`crates/hya-server/src/opencode/skill_catalog.rs`](../crates/hya-server/src/opencode/skill_catalog.rs).
...
skill. This is similar to hya/OpenCode's progressive-disclosure approach, but
...
stock-core abstraction in the way it is for hya and OpenCode.
...
OpenCode exposes a broad, unified tool plane. Built-ins, standalone JS/TS custom
...
Skills are loaded through a native `skill` tool. OpenCode discovers skill
...
MCP is a first-class OpenCode feature. Config supports local and remote servers,
...
Evidence: OpenCode [tools](https://opencode.ai/docs/tools),
[custom tools](https://opencode.ai/docs/custom-tools),
[skills](https://opencode.ai/docs/skills),
[MCP servers](https://opencode.ai/docs/mcp-servers),
[`packages/opencode/src/tool/registry.ts`](https://raw.githubusercontent.com/anomalyco/opencode/dev/packages/opencode/src/tool/registry.ts),
and [`packages/plugin/src/tool.ts`](https://raw.githubusercontent.com/anomalyco/opencode/dev/packages/plugin/src/tool.ts).
...
narrower than Pi/OpenCode in out-of-the-box provider breadth. The user supplies
...
OpenCode has the broadest documented provider surface of the three. Official
...
and OpenCode's own provider surface.

OpenCode's provider layer is not just a static list. Source has provider-specific
...
Evidence: OpenCode [providers](https://opencode.ai/docs/providers),
[CLI](https://opencode.ai/docs/cli),
[`packages/opencode/src/provider/provider.ts`](https://raw.githubusercontent.com/anomalyco/opencode/dev/packages/opencode/src/provider/provider.ts),
and [`packages/core/package.json`](https://raw.githubusercontent.com/anomalyco/opencode/dev/packages/core/package.json).
...
OpenCode-style first-class subagent sessions.
...
OpenCode has first-class primary agents and subagents. Built-in primary agents
...
multi-agent work visible to users. Compared with hya, OpenCode's subagent UX is
...
Evidence: OpenCode [agents](https://opencode.ai/docs/agents),
[keybinds](https://opencode.ai/docs/keybinds),
[CLI](https://opencode.ai/docs/cli), and
[`packages/opencode/src/agent/agent.ts`](https://raw.githubusercontent.com/anomalyco/opencode/dev/packages/opencode/src/agent/agent.ts).
...
`/mcp`, `/export`, and `/compact`. OpenCode parity tracking shows hya implements
some OpenCode-compatible TUI HTTP control routes, but still lacks full OpenCode
...
[OpenCode Parity Matrix](opencode-parity.md),
...
OpenCode's TUI emphasizes session ergonomics and control. It supports `@` fuzzy
...
OpenCode also exposes remote/control surfaces: CLI `attach`, server `/tui/*`
...
Evidence: OpenCode [TUI](https://opencode.ai/docs/tui),
[keybinds](https://opencode.ai/docs/keybinds),
...
skill directory for native runtime discovery, or an OpenCode-style skill
directory when targeting the OpenCode-compatible server catalog. Letting the
...
hya also ships `crates/hya-plugin-opencode`, a Bun/TypeScript adapter for
OpenCode plugins. That path gives hya compatibility with much of OpenCode's
plugin ecosystem while still feeding the Rust runtime's event, permission, and
tool planes.

Evidence: [Configuration](configuration.md),
[Tools and Permissions](architecture/tools-and-permissions.md),
[OpenCode adapter README](../crates/hya-plugin-opencode/README.md),
...
OpenCode's first-class MCP config or hya's `mcp:` runtime composition.
...
OpenCode has several extension layers:
...
- markdown skills in OpenCode/Claude/Agents-compatible directories
...
tool plane. Plugins can also use the OpenCode SDK client to interact with app,
MCP, auth, TUI, and other server surfaces where supported.

Evidence: OpenCode [plugins](https://opencode.ai/docs/plugins),
[custom tools](https://opencode.ai/docs/custom-tools),
[commands](https://opencode.ai/docs/commands),
[skills](https://opencode.ai/docs/skills),
[MCP servers](https://opencode.ai/docs/mcp-servers),
[config](https://opencode.ai/docs/config),
[`packages/plugin/src/index.ts`](https://raw.githubusercontent.com/anomalyco/opencode/dev/packages/plugin/src/index.ts),
[`packages/plugin/src/tool.ts`](https://raw.githubusercontent.com/anomalyco/opencode/dev/packages/plugin/src/tool.ts),
[`packages/opencode/src/plugin/loader.ts`](https://raw.githubusercontent.com/anomalyco/opencode/dev/packages/opencode/src/plugin/loader.ts),
and [`packages/opencode/src/tool/registry.ts`](https://raw.githubusercontent.com/anomalyco/opencode/dev/packages/opencode/src/tool/registry.ts).

## Practical Takeaways

- Pick **hya** when you want a Rust event-sourced runtime with unified tool,
  permission, plugin, MCP, and multi-agent machinery, and when OpenCode API
...
- Pick **OpenCode** when you want the broadest out-of-the-box provider catalog,
...
customizability and extensionable TUI/provider workflows; OpenCode is strongest
