...
| Surfaces | [`hya-cli`](../../crates/hya-cli), [`hya-server`](../../crates/hya-server), [`hya-client`](../../crates/hya-client), [`hya-render-tui`](../../crates/hya-render-tui), [`hya-plugin-opencode`](../../crates/hya-plugin-opencode) | Expose the runtime through CLI, TUI, native/OpenCode HTTP/SSE, typed client APIs, and the OpenCode plugin adapter. |
...
- `hya run` is an OpenCode-compatible alias for headless prompt execution.
- `hya -p` runs goal mode with an independent model-backed evaluator.
- `hya serve` exposes HTTP and SSE over the same engine.
- `hya tail-session` replays JSON envelopes from a persisted SQLite event log.
- `hya models`, `login`, `auth`/`providers`, `agent`, `sessions`, and `rpc`
