# Per-model reasoning variants with Compat parity
...
to the provider, with **per-model validity**, mirroring Compat's config schema
...
- Agent frontmatter `variant` + `options` are parsed and stored in the compat
  ...
  `system_prompt`+`name` (`yaca-server/src/compat/reference.rs:19-40`).
...
- yaca-server compat layer already loads `opencode.json`/`opencode.jsonc`
  (`global.rs:27`, `agent_sources.rs:133-136`).

See `research/compat-reasoning-schema.md` for the Compat schema + citations.

## Scope (chosen: full Compat parity)

1. **Reasoning vocabulary + per-provider validity.** Support Compat's level set
...
2. **Per-model config from `opencode.json`.** Read
   ...
   as provider-option bundles, mirroring Compat.
3. **Agent variant resolution.** An agent's `variant:` (frontmatter / opencode.json
   ...
   pass through as model options, per Compat.
4. **Provider request mapping.** Apply resolved options to the request:
   Anthropic `thinking` (high=16000, max=31999 — Compat's constant for ≥32k-output models),
...
- R2: The reasoning level abstraction covers the Compat vocabulary with a
  per-provider validity gate; OpenAI never receives `max`.
- R3: Anthropic budgets align to Compat: `high`→16000, `max`→31999. 31999 is
  Compat's value for models with ≥32k output, which covers all configured
...
- R4: Per-model `options` and `variants` are read from `opencode.json` /
  ...
  (reuse the existing compat config load path).
...
- [ ] AC3: A unit/integration test proves an `opencode.json`
  ...
  body (Anthropic `thinking.budget_tokens` == 31999, the Compat constant for
  ≥32k-output models).
...
- Migrating yaca's native `config.yaml` to JSON; this task reads reasoning from
  the Compat config that the server layer already loads.

## Constraints

- Verification gate: `cargo fmt --all --check && cargo clippy --workspace --all-targets -- -D warnings && cargo test --workspace`.
- For Compat adapter changes (if any): `bun run typecheck && bun test` in
  `crates/yaca-plugin-compat/adapter`.
...
- DQ1: Keep the simple `ReasoningEffort` enum (extended with the full level set +
  a provider-validity function) vs. adopt Compat's richer "variant = arbitrary
...
- DQ2: Where variant resolution lives (yaca-core engine vs. yaca-server compat
