...
Child of `06-20-yaca-pi-parity`. See parent `design.md` / `implement.md` (Wave 1) for
full design. This is the P0 foundation: today yaca **cannot** edit/write/run shell in
any mode.
...
- BUG: `yaca-cli/src/main.rs::build_session_engine` drops the Ask receiver
  (`let (permission, _asks) = …`); Edit/Bash → `Mode::Ask` → dead channel → fail.
- **The TUI overlay ALREADY EXISTS** and is rendered: `yaca-tui/src/lib.rs`
...
- `yaca-tool` already exports `AskRequest`, `Decision`, `Action`, `Mode`, etc.
...
- R2. Decoupling: keep `yaca-tui` pure — do NOT put tokio/oneshot in `PermissionPrompt`;
...
## Acceptance criteria
- [ ] AC1. `yaca exec "write 'hi' to ./qa.txt"` (real model) creates the file; an
