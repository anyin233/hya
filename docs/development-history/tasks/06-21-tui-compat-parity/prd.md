# Refactor Rust TUI toward compat parity

## Goal

Bring the Rust `yaca` TUI much closer to Compat's terminal UI in visible
...
- Study the upstream Compat TUI source and use it as the reference for
...
- Add paste handling inspired by Claude Code/Compat:
...
- Improve transcript spacing so user messages, assistant text, reasoning, tools,
  system messages, and errors have deliberate separation similar to Compat.
- Replace the current generic tool line with compat-inspired inline/block tool
  rows:
...
- [x] Planning artifacts (`prd.md`, `design.md`, `implement.md`, and research
      notes) cite the Compat reference paths and the yaca files to change.
...
- Local repository did not contain a `reference/` directory. Reference source is
  the upstream `anomalyco/compat` repository cloned to
  `/tmp/yaca-compat-ref` at commit `f12ac6f234ebe31982ee78f3359e8170cb09ffc9`.
