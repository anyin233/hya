# compat TUI — consolidated reference inventory
...
## 1. Architecture (compat) → proposed yaca module mapping
...
## 2. Appearance — default theme `compat` (dark mode, shipped startup look)
...
7. **Question flow** (if adopting compat question UX) — event + decision endpoint.
...
- **W7 — Fidelity QA**: side-by-side visual QA vs compat (tmux), keymap parity audit, polish.
