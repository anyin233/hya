...
**Real example**: An update path had a manual file list for 11 scripts that an
existing `getAllScripts()` helper already tracked. Fix: replaced the manual list
with a `for..of getAllScripts()` loop in the update path.
