...
When the same command or configuration template exists across multiple platform
integrations, treat it as a cross-layer boundary. A change in one copy must be
reviewed against every equivalent representation.

### Checklist: After Modifying Any Cross-Platform Template

- [ ] Find all platform copies of the command or template.
- [ ] Update every representation while preserving its syntax (Markdown, TOML,
      or another format).
- [ ] Check that shared behavior and platform-specific escaping remain aligned.
- [ ] Run the repository's focused cross-layer checks, or read the relevant
      guidance directly, before finalizing.

**Real-world example**: Updated one platform's session-recording template but
forgot the equivalent copies in other integrations. A cross-layer check caught
the drift before release.

---

## Generated Runtime Template Upgrade Consistency

Some generated files are both documentation and runtime input. Treat their
writer, parser, fresh-install path, and upgrade path as one contract. Template
changes must be validated against both fresh installs and upgrades.

### Checklist: After Modifying A Runtime-Parsed Template

- [ ] Identify every runtime parser that reads the template, not just the file
      writer that installs it.
- [ ] Check whether relevant syntax lives outside obvious managed regions such
      as tag blocks.
- [ ] Verify fresh installation output and a versioned update scenario.
...
When a CLI auto-detects a mode by probing a remote resource (e.g., checking if
`index.json` exists to decide marketplace vs direct download):
...
**Real-world example**: Agent-session update hints fetched package `latest`
metadata with `response.read(4096)` and then parsed it as complete JSON. The
response exceeded 4 KB, so the JSON was truncated, parse failed silently, and the
first session injection showed no update hint. Fix: read the complete response
before parsing, and add a regression where `version` is followed by an 8 KB
metadata tail.
