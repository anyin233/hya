...
use yaca_core::{TmuxPaneManager, WorktreeManager};
use yaca_proto::TeamRunId;
...
fn tempdir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("yaca-wt-{nanos}-{}", std::process::id()));
...
async fn tmux_capability_probe_and_degrade() {
    ...
    if !available {
        let mgr = TmuxPaneManager::new("yaca-test");
