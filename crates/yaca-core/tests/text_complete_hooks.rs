...
use yaca_proto::{AgentName, Envelope, Event, FinishReason, ModelRef, PartProjection, Role};
...
impl HookDispatcher for TextCompleteHost {
    ...
    async fn command_execute_before(
        &self,
        input: yaca_core::CommandExecuteBeforeInput,
    ) -> yaca_core::CommandExecuteBeforeOutcome {
        yaca_core::CommandExecuteBeforeOutcome::Continue { text: input.text }
    }
