    ...
    for part in parts {
        match part {
            ...
            TimelinePart::Tool { name, status } => {
                lines.push(Line::from(vec![
                    Span::raw("  "),
                    Span::styled("• ", Style::default().fg(status.color(theme))),
                    Span::styled(
...
fn sidebar_lines(state: &AppState, theme: &Theme) -> Vec<Line<'static>> {
    ...
    if let Some(permission) = &state.pending_permission {
        ...
        lines.push(Line::from(permission.clone()));
    }
    lines
}

pub fn render_prompt(frame: &mut Frame, area: Rect, state: &AppState, theme: &Theme) {
    let widget = match &state.pending_permission {
        Some(req) => Paragraph::new(format!("PERMISSION REQUEST: {req}  [a]llow  [d]eny"))
            .block(
                Block::default()
                    .title(" permission ")
                    .borders(Borders::ALL)
...
pub fn prompt_cursor(state: &AppState, area: Rect) -> Option<(u16, u16)> {
    if state.pending_permission.is_some() || state.running {
        return None;
    }
