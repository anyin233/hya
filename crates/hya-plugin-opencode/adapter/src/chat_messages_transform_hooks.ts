...
export async function runChatMessagesTransformHooks(
  ...
  return output.messages.map((entry, index) =>
    hyaMessageFromOpenCodeEntry(entry, wireMessages[index]),
  )
}
...
function hyaMessageFromOpenCodeEntry(
  entry: unknown,
  ...
  const parts = entry.parts.map((part) => hyaPartFromOpenCodePart(part, original?.parts ?? []))
...
function hyaPartFromOpenCodePart(part: unknown, originalParts: readonly unknown[]): unknown {
  if (!isTextPart(part)) {
    return part
  }
