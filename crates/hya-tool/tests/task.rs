...
#[tokio::test]
async fn task_rejects_the_removed_name_parameter() {
    let parent = SessionId::new();
    let (spawner, mut rx) = SpawnerPlane::new();
    let ctx = ctx_with_session(vec![allow(Action::Task, "*")], spawner, parent);
    let tool = ToolRegistry::builtins().get("task").unwrap();
    for input in [
        json!({"description": "d", "prompt": "p", "name": "scout"}),
        json!({"description": "d", "prompt": "p", "subagent_type": "explore", "name": ""}),
        json!({"members": [
            {"prompt": "scan", "subagent_type": "explore"},
            {"prompt": "build", "subagent_type": "general", "name": "dev"}
        ]}),
    ] {
        let error = tool.execute(&ctx, input.clone()).await.unwrap_err();
        let ToolError::Input(message) = error else {
            panic!("{input}: must be an input error, got {error:?}");
        };
        assert!(
            message.contains("`name` was removed")
                && message.contains("subagent_type")
                && message.contains("<subagent_type>-<operator>"),
            "{message}"
        );
    }
...
#[test]
fn task_schema_chooses_the_agent_by_subagent_type_only() {
    let tool = ToolRegistry::builtins().get("task").unwrap();
