...
test("initialize passes Compat shell and project time to local plugins", async () => {
...
test("initialize passes an Compat app log client to local plugins", async () => {
  const root = await makeTempDir()
...
test("initialize passes Compat project and path clients to local plugins", async () => {
  const root = await makeTempDir()
  ...
  await writeFile(
    ...
    [
      ...
      `    if (paths.data.config !== ${JSON.stringify(path.join(configRoot, "compat"))}) throw new Error("config mismatch")`,
      `    if (paths.data.state !== ${JSON.stringify(path.join(stateRoot, "compat"))}) throw new Error("state mismatch")`,
      "    return { event: async () => {} }",
      "  },",
      "}",
    ].join("\n"),
...
test("initialize passes an Compat vcs client to local plugins", async () => {
  const root = await makeTempDir()
