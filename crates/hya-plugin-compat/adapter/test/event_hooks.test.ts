...
test("event notification fans out to Compat event hooks", async () => {
  ...
  await writeFile(
    ...
    [
      ...
      "  server: async () => ({",
      "    event: async (input) => {",
      "      await writeFile(process.env.HYA_EVENT_MARKER, JSON.stringify(input.event))",
      "    },",
      "  }),",
      "}",
    ].join("\n"),
...
test("command executed events map to Compat command events", async () => {
  const root = await makeTempDir()
...
async function makeTempDir(): Promise<string> {
  const root = await mkdtemp(path.join(tmpdir(), "hya-compat-event-"))
  tempDirs.push(root)
