...
test("experimental.chat.messages.transform rewrites hya chat messages during chat params", async () => {
  // Given: an Compat plugin that mutates the first text part in message history.
  ...
  // When: hya asks the adapter to run chat.params for the same request history.
  ...
  // Then: the Compat transform hook is advertised and its text mutation is preserved.
...
test("experimental.chat.messages.transform preserves assistant metadata when rewriting text", async () => {
  // Given: an assistant message using hya metadata that differs from Compat metadata.
  const root = await makeTempDir()
  ...
  // Then: hya assistant metadata is preserved while the text mutation is applied.
  expect(responses[1]?.result).toEqual({
    outcome: "continue",
    request: {
      model: "gpt-5",
...
async function makeTempDir(): Promise<string> {
  const root = await mkdtemp(path.join(tmpdir(), "hya-compat-chat-messages-"))
  tempDirs.push(root)
