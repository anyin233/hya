...
import { runChatMessagesTransformHooks } from "./chat_messages_transform_hooks"
...
type SystemTransformHook = (
  input: { readonly sessionID?: string; readonly model: CompatModel },
...
type ProviderContext = {
  ...
  readonly info: {
    ...
    readonly options: Record<string, unknown>
    readonly models: Record<string, CompatModel>
  }
  readonly options: Record<string, unknown>
}
...
export async function runChatParamsHooks(
  ...
  const model = compatModel(params.request.model)
...
function chatParamsInput(params: ChatParamsParams, model: CompatModel): ChatParamsInput {
  return {
...
function compatModel(modelRef: string): CompatModel {
  const slash = modelRef.indexOf("/")
  const providerID = slash < 0 ? "hya" : modelRef.slice(0, slash)
...
function providerContext(model: CompatModel): ProviderContext {
  return {
    source: "custom",
    info: {
      id: model.providerID,
