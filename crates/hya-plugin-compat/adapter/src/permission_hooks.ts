...
export async function runPermissionAskHooks(
  ...
  const input = compatPermission(params)
...
function compatPermission(params: PermissionAskParams): CompatPermission {
  const pattern = resourcePattern(params.resource)
  const sessionID = params.session ?? ""
  return {
    id: `perm_hya_${params.action}_${pattern}`,
    type: params.action,
    pattern,
    sessionID,
    messageID: "",
