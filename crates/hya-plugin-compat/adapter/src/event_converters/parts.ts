...
export function toolCallRequestedEvent(envelope: EventEnvelope): CompatEvent | undefined {
...
export function toolInputStartEvent(envelope: EventEnvelope): CompatEvent | undefined {
  const ids = toolIds(envelope.event)
  const tool = stringField(envelope.event, "name")
  if (ids === undefined || tool === undefined) {
    return undefined
...
export function toolInputDeltaEvent(envelope: EventEnvelope): CompatEvent | undefined {
  const ids = toolIds(envelope.event)
  const tool = stringField(envelope.event, "name")
...
export function toolResultEvent(envelope: EventEnvelope): CompatEvent | undefined {
  const ids = toolIds(envelope.event)
  if (ids === undefined) {
    return undefined
  }
...
export function toolErrorEvent(envelope: EventEnvelope): CompatEvent | undefined {
  const ids = toolIds(envelope.event)
