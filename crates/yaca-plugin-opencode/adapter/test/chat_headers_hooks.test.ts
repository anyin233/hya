...
test("chat.headers adds OpenCode plugin headers during chat params", async () => {
  // Given: an OpenCode plugin that derives request headers from chat context.
  ...
  await writeFile(
    ...
    [
      ...
      '      output.headers["x-yaca-session"] = input.sessionID',
      '      output.headers["x-yaca-model"] = input.model.id',
      "    },",
      "  }),",
      "}",
    ].join("\n"),
  ...
  // When: yaca asks the adapter to prepare chat parameters.
  ...
  // Then: the OpenCode chat.headers hook is exposed through yaca chat.params.
  ...
  expect(responses[1]?.result).toEqual({
    ...
    request: {
      ...
      temperature: 0,
      headers: {
        "x-yaca-session": "session-headers",
        "x-yaca-model": "gpt-5",
      },
    },
  })
...
async function makeTempDir(): Promise<string> {
  const root = await mkdtemp(path.join(tmpdir(), "yaca-opencode-chat-headers-"))
  tempDirs.push(root)
