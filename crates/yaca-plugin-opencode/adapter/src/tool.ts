...
export type ToolRegistry = {
  ...
  readonly tools: ReadonlyMap<string, OpenCodeToolDefinition>
}

export type OpenCodeToolDefinition = {
  readonly description: string
...
export class UnsupportedToolAskError extends Error {
  ...
  constructor() {
    super("OpenCode tool context ask() is not supported by yaca adapter yet")
  }
}

export function buildToolRegistry(hooks: readonly OpenCodeHooks[]): ToolRegistry {
  const infos: ToolInfo[] = []
  const tools = new Map<string, OpenCodeToolDefinition>()
...
export async function callRegisteredTool(
  registry: ReadonlyMap<string, OpenCodeToolDefinition>,
...
function isToolDefinition(value: unknown): value is OpenCodeToolDefinition {
  return (
    isRecord(value) &&
