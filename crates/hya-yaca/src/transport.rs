//! In-process native transport: drives the in-process yaca `axum::Router` via
...
/// A [`Transport`] that routes every request through the in-process yaca `Router` instead of HTTP.
///
...
mod tests {
    ...
    async fn offline_runtime() -> YacaRuntime {
    ...
    fn native_client(rt: &YacaRuntime) -> YacaNativeClient {
        ApiClient::with_transport(YacaNativeTransport::new(rt.router().clone(), "/tmp"))
    }

    #[tokio::test]
    async fn get_config_returns_object() {
        let rt = offline_runtime().await;
        let transport = YacaNativeTransport::new(rt.router().clone(), "/tmp");
        let value = transport
    ...
    async fn directory_is_carried_and_request_succeeds() {
        let rt = offline_runtime().await;
        let transport = YacaNativeTransport::new(rt.router().clone(), "/tmp/hya-test-dir");
