...
//!
//! Handles are `<prefix>-<operator>` with a random operator name; the backend
//! runs with `HYA_HANDLE_SEED` so the scripted lead can address the worker.
...
async fn t2_31_wait_returns_when_the_worker_reports_inside_the_lead_turn() {
    let env = E2eEnvBuilder::new()
        .backend_env("HYA_HANDLE_SEED", SEED.to_string())
        .route(
            WORKER,
            vec![tool_step(
                "report",
        ...
        .route(
            ...
            vec![
                ...
                tool_step(
                    "wait",
                    json!({"targets": [worker_handle()], "timeout_secs": 60}),
                ),
...
async fn t2_31_archive_stops_a_worker_and_mail_wakes_it() {
    let env = E2eEnvBuilder::new()
        ...
        .route(
            ...
            vec![
                ...
                tool_step(
                    "archive",
                    json!({"target": worker_handle(), "reason": "pause"}),
                ),
                tool_step(
                    "send",
                    json!({"channel": worker_handle(), "body": "WAKE_UP_PLEASE"}),
                ),
                text_step("ROOT_DONE"),
                text_step("ROOT_EXTRA_1"),
                text_step("ROOT_EXTRA_2"),
    ...
    env.wait_route_contains(ROOT, &format!("Archived `{}`", worker_handle()), timeout)
        .await
