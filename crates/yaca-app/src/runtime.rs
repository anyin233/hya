...
pub async fn build_session_engine(
    ...
    yaca_mcp::McpManager,
    Arc<yaca_plugin::PluginHost>,
) {
    let router = Arc::new(router);
    let mut registry = ToolRegistry::builtins();
    let mcp_manager = yaca_mcp::McpManager::connect_all(mcp).await;
    for tool in mcp_manager.tools() {
        if let Err(error) = registry.register(tool) {
            eprintln!("yaca: skipping MCP tool ({error})");
        }
    }
    let plugin_host = Arc::new(yaca_plugin::PluginHost::connect_all(plugins, host_info()).await);
    ...
    let permission = if plugin_host.is_empty() {
        ...
        permission.with_interceptor(Arc::new(yaca_plugin::PermissionBridge::new(
            plugin_host.clone(),
        )))
    };
...
pub struct YacaRuntime {
    router: axum::Router,
    engine: Arc<SessionEngine>,
    app_state: yaca_server::AppState,
...
impl YacaRuntime {
    pub async fn start(opts: RuntimeOptions) -> anyhow::Result<Self> {
        ...
        .await;
        let mut state =
            yaca_server::AppState::new(engine.clone(), Arc::new(agent_with_model(&runtime.model)))
        ...
        let _permission_responder = if opts.yolo {
            eprintln!(
                "yaca: --yolo auto-approves ALL tool actions for the hya frontend (RCE risk)"
            );
