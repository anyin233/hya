...
//! pure `hya-render-tui` crate. A spawned task runs each turn and streams its events
...
use hya_render_tui::{AppState, PermissionPrompt, QuestionPrompt};
use hya_tool::{
...
fn reference_items(workdir: &Path) -> Vec<hya_render_tui::DialogItem> {
    ...
    while let Some((dir, depth)) = stack.pop() {
        ...
        for entry in paths {
            ...
            items.push(hya_render_tui::DialogItem {
                label,
...
pub async fn run(
    ...
    terminal
        .draw(|f| hya_render_tui::draw(f, &mut controller.app))
        .context("draw")?;
