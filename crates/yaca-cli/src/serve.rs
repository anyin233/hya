...
/// Default `yaca`: run the HTTP/SSE backend in-process on an ephemeral loopback port and hand the
/// terminal to the `hya` frontend (the replacement for `yaca-tui`). The legacy TUI stays reachable
/// via `yaca --mini`.
pub(crate) async fn cmd_tui_hya(
    ...
    if !std::io::stdout().is_terminal() {
        ...
        println!(
            "The hya frontend needs a terminal. Try `yaca exec \"<prompt>\"`, \
    ...
    let mut state = AppState::new(engine, Arc::new(agent_with_model(&runtime.model)))
        ...
        .with_global_agents(true);
    let _responder = if yolo {
        eprintln!("yaca: --yolo auto-approves ALL tool actions for the hya frontend (RCE risk)");
        Some(spawn_auto_responder(asks, PermissionPolicy::Yolo))
    } else {
        state = state.with_permission_requests(asks);
        None
...
async fn launch_hya(base_url: &str) -> anyhow::Result<()> {
    ...
    let status = tokio::process::Command::new(&bin)
        ...
        .status()
        .await
        .with_context(|| format!("launch hya frontend `{bin}` (set YACA_HYA_BIN to override)"))?;
    if !status.success() {
...
/// Resolve the `hya` binary: `YACA_HYA_BIN`, then the most recently built sibling
...
fn resolve_hya_bin() -> String {
    if let Ok(bin) = std::env::var("YACA_HYA_BIN") {
        return bin;
    }
