...
use yaca_proto::{SessionId, ToolSchema};
...
impl Tool for TaskTool {
    ...
    fn schema(&self) -> ToolSchema {
        obj_schema(
            "task",
            "Launch a specialized subagent for a complex task. Use task_id to resume a prior subagent session; background launches are accepted by schema but currently require foreground execution in yaca.",
            json!({
                "description": {
                    "type": "string",
                ...
                "members": {
                    "type": "array",
                    "description": "yaca extension: dispatch several members in one tool call",
                    "items": {
                        "type": "object",
                        "properties": {
