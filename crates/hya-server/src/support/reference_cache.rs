...
pub(super) fn materialize(repository: &str, branch: Option<&str>, path: PathBuf) {
    let Some(reference) = crate::support::reference_repository::parse(repository) else {
        return;
    };
    let remote = crate::support::reference_repository::remote(&reference).to_string();
    ...
    tokio::spawn(async move {
        if !wait_for_turn(&path).await {
            tracing::warn!(
                repository = %remote,
                path = %path.display(),
                "timed out waiting to materialize compat reference"
            );
            return;
        }
        if let Err(error) = ensure(&remote, branch.as_deref(), &path).await {
            tracing::warn!(
                ...
                repository = %remote,
                path = %path.display(),
                "failed to materialize compat reference"
            );
        }
        unmark_active(&path);
    });
