...
pub(super) fn reference_entries(config: &Value) -> Option<&Map<String, Value>> {
    config
        .get("references")
...
pub(super) fn reference(name: &str, entry: &Value, base: &Path) -> Option<Value> {
    match entry {
...
pub(super) fn materialize_git(references: &[Value]) {
    for reference in references {
        ...
        crate::support::reference_cache::materialize(
            repository,
...
pub(super) fn valid_alias(name: &str) -> bool {
    !name.is_empty()
        && !name
            .chars()
...
fn git_reference(
    ...
    if let Some(branch) = branch
        && !crate::support::reference_repository::valid_branch(branch)
    {
        return None;
    }
    let parsed = crate::support::reference_repository::parse(repository)?;
    let path = crate::support::reference_repository::cache_path(&parsed)
        .to_string_lossy()
        .into_owned();
