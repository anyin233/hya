...
async fn list(
    ...
    sessions.retain(|session| !session.empty_unnamed());
    if order == Some("asc") {
        sessions.reverse();
    }
...
async fn create(
    State(st): State<ServerState>,
    body: Bytes,
) -> Result<Json<DataResponse<CompatSessionInfo>>, ApiError> {
...
async fn load_sessions(st: &ServerState) -> Result<Vec<CompatSessionInfo>, ApiError> {
    let sessions = st
        .engine
        .store()
        .list_sessions()
...
pub(in crate::compat) async fn load_existing_session(
    st: &ServerState,
    session: SessionId,
    id: &str,
) -> Result<Result<super::projection::CompatSessionSnapshot, Response>, ApiError> {
