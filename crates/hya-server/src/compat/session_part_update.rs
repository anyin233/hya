...
fn parse_tool_state(state: Value) -> Result<ToolPartState, ApiError> {
    serde_json::from_value::<ToolPartState>(state.clone())
        .or_else(|_| serde_json::from_value::<CompatToolState>(state).map(Into::into))
        .map_err(|_| ApiError::bad_request("invalid tool state"))
}

#[derive(Deserialize)]
#[serde(tag = "status", rename_all = "lowercase")]
enum CompatToolState {
    Pending {
        input: Value,
    },
    Running {
...
#[derive(Deserialize)]
struct CompatToolTime {
    start: Option<u64>,
    end: Option<u64>,
}

impl From<CompatToolState> for ToolPartState {
    fn from(state: CompatToolState) -> Self {
        match state {
            CompatToolState::Pending { input } => Self::Pending { input },
            CompatToolState::Running { input } => Self::Running { input },
            CompatToolState::Completed {
                input,
                output,
                time,
            } => Self::Completed {
            ...
            CompatToolState::Error { input, error } => Self::Error {
                input,
...
fn elapsed_ms(time: Option<&CompatToolTime>) -> u64 {
