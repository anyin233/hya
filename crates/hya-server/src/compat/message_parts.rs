...
pub(super) struct CompatPartContext {
    times: BTreeMap<PartId, CompatPartTime>,
    timeline: BTreeMap<MessageId, Vec<CompatPartCursor>>,
    deleted: BTreeMap<MessageId, BTreeSet<PartId>>,
}

#[derive(Clone, Copy)]
enum CompatPartCursor {
    Part(PartId),
    StepStart { step: u32 },
    StepFinish { step: u32, finish: FinishReason },
}

impl CompatPartContext {
    ...
    fn time(&self, part: PartId) -> CompatPartTime {
...
fn part_timeline(envs: &[Envelope]) -> BTreeMap<MessageId, Vec<CompatPartCursor>> {
    let mut out: BTreeMap<MessageId, Vec<CompatPartCursor>> = BTreeMap::new();
    for env in envs {
        match &env.event {
            Event::StepStarted { message, step, .. } => {
                out.entry(*message)
                    .or_default()
                    .push(CompatPartCursor::StepStart { step: *step });
            }
            Event::StepFinished {
                message,
                step,
                ...
                out.entry(*message)
                    .or_default()
                    .push(CompatPartCursor::StepFinish {
                        step: *step,
                        finish: *finish,
                    });
            }
            ...
            | Event::ToolPartUpdated { message, part, .. } => {
                out.entry(*message)
                    .or_default()
                    .push(CompatPartCursor::Part(*part));
            }
...
fn part_times(envs: &[Envelope]) -> BTreeMap<PartId, CompatPartTime> {
...
fn mark_start(out: &mut BTreeMap<PartId, CompatPartTime>, part: PartId, time: u64) {
    out.entry(part).or_default().start.get_or_insert(time);
}

fn mark_end(out: &mut BTreeMap<PartId, CompatPartTime>, part: PartId, time: u64) {
    let entry = out.entry(part).or_default();
    entry.start.get_or_insert(time);
    entry.end = Some(time);
}

fn mark_completion_end(out: &mut BTreeMap<PartId, CompatPartTime>, part: PartId, time: u64) {
    out.entry(part).or_default().end = Some(time);
}

fn update_tool_time(
    out: &mut BTreeMap<PartId, CompatPartTime>,
    part: PartId,
    state: &ToolPartState,
    time: u64,
) {
...
fn time_value(time: CompatPartTime) -> Value {
...
fn running_time(time: CompatPartTime) -> Value {
    json!({ "start": time.start.unwrap_or(0) })
}

fn completed_time(time: CompatPartTime, fallback_elapsed: u64) -> Value {
