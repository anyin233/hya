...
use hya_provider::{AnthropicMessagesProtocol, CompletionRequest, Protocol, ReasoningEffort};
use serde_json::{Value, json};

use super::reasoning_options::{load_compat_config, resolve_reasoning};
...
fn tempdir() -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!(
        "hya-server-reasoning-options-test-{nanos}-{}",
        std::process::id()
    ));
...
"#,
    ...
    let config = load_compat_config(&workdir);
