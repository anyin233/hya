...
#[derive(Clone)]
pub(super) struct CompatModelRefParts {
...
#[derive(Deserialize)]
pub(super) struct CompatModelRefRequest {
    id: String,
    #[serde(rename = "providerID")]
    provider_id: String,
...
pub(super) fn model_ref_parts(model: &ModelRef) -> CompatModelRefParts {
    let (raw, variant) = split_variant(model.as_str());
    if let Some((provider_id, model_id)) = raw.split_once('/') {
        return CompatModelRefParts {
...
///
/// Accepts the Compat object form (`{ providerID, modelID | id, variant? }`) or
...
pub(super) fn model_info(model: &ModelRef) -> CompatModel {
