//! Integration tests for `hya-server`: compat location api.
...
#[tokio::test]
async fn compat_location_query_and_headers_override_default_workdir() {
    let workdir = tempdir();
...
#[tokio::test]
async fn compat_location_accepts_workspace_routing_query_names() {
    let workdir = tempdir();
...
#[tokio::test]
async fn compat_location_workspace_query_routes_to_worktree_directory() {
    if !git_available() {
