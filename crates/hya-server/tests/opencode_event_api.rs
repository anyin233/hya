...
const WORKDIR: &str = "/tmp/hya-opencode-event-api";
...
async fn opencode_v2_event_route_streams_connected_event() {
    ...
    let resp = app
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/api/event?location%5Bdirectory%5D=/tmp/hya-opencode-event-api")
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
async fn opencode_v2_event_route_streams_session_created_location() {
    let app = router(state().await);
    let directory = "/tmp/hya-opencode-event-api-scoped";
    let resp = app
        .clone()
        .oneshot(
            Request::builder()
...
async fn opencode_v2_event_route_filters_session_events_by_location() {
    let app = router(state().await);
    let visible = "/tmp/hya-opencode-event-api-visible";
    let hidden = "/tmp/hya-opencode-event-api-hidden";
    let resp = app
        .clone()
        .oneshot(
            Request::builder()
