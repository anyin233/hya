//! Integration tests for `hya-server`: compat project copy api.
...
#[tokio::test]
async fn compat_project_copy_creates_and_removes_git_worktree() {
    if !git_available() {
...
#[tokio::test]
async fn compat_project_copy_rejects_unknown_strategy() {
    if !git_available() {
        eprintln!("skipping: git is not available");
        return;
    }
...
#[tokio::test]
async fn compat_project_copy_generates_short_names() {
    if !git_available() {
        eprintln!("skipping: git is not available");
        return;
    }
...
#[tokio::test]
async fn compat_project_routes_return_current_directory() {
    let repo = init_repo();
    let app = router(state(repo.clone()).await);
...
#[tokio::test]
async fn compat_project_git_init_creates_repository_and_returns_project() {
    if !git_available() {
        eprintln!("skipping: git is not available");
        return;
    }
...
#[tokio::test]
async fn compat_project_git_init_creates_repository_inside_parent_worktree() {
    if !git_available() {
        eprintln!("skipping: git is not available");
        return;
    }
...
#[tokio::test]
async fn compat_project_git_init_repairs_empty_git_marker_directory() {
    if !git_available() {
        eprintln!("skipping: git is not available");
        return;
    }
...
#[tokio::test]
async fn compat_project_update_persists_runtime_metadata() {
    ...
    assert_eq!(current.status(), StatusCode::OK);
    let current_body = body_json(current).await;
    assert_eq!(current_body["name"], "Hya");
