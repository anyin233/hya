//! Integration tests for `hya-server`: compat session v2 context time api.
...
const WORKDIR: &str = "/tmp/hya-compat-session-v2-context-time-api";
...
#[tokio::test]
async fn compat_v2_context_omits_unfinished_assistant_completed_time() {
    ...
    .await
    .expect("prompt route should return without waiting for provider stream");
    assert_eq!(status, StatusCode::OK);
