//! Integration tests for `hya-server`: compat session v2 create api.
...
fn workdir() -> &'static str {
    ...
    WORKDIR.get_or_init(|| {
        let dir = std::env::temp_dir().join("hya-compat-session-v2-create-api");
...
#[tokio::test]
async fn compat_v2_session_create_accepts_empty_body() {
...
#[tokio::test]
async fn compat_v2_session_create_treats_whitespace_body_as_empty() {
    let app = router(state().await);
...
#[tokio::test]
async fn compat_v2_session_create_accepts_parent_id() {
    let app = router(state().await);
