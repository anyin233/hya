//! Integration tests for `hya-server`: compat experimental session filter api.
...
const WORKDIR: &str = "/tmp/hya-compat-experimental-session-filter-api";
const OTHER_WORKDIR: &str = "/tmp/hya-compat-experimental-session-filter-api-other";
...
async fn experimental_session_list_filters_archived_sessions_by_default() {
    ...
    let active_id = compat_id(&active);
    let archived_id = compat_id(&archived);
...
async fn experimental_session_list_uses_updated_time_cursor() {
    ...
    assert_eq!(
        ...
        vec![compat_id(&newest), compat_id(&middle)]
    );
    ...
    assert_eq!(session_ids(&second_page), vec![compat_id(&oldest)]);
}
...
async fn experimental_session_list_filters_directory() {
    ...
    assert!(ids.contains(&compat_id(&included)));
    assert!(!ids.contains(&compat_id(&excluded)));
}
