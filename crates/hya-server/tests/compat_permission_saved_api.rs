//! Integration tests for `hya-server`: compat permission saved api.
...
const WORKDIR: &str = "/tmp/hya-compat-permission-saved-api";
...
#[tokio::test]
async fn compat_permission_saved_lists_and_removes_always_reply() {
...
#[tokio::test]
async fn compat_permission_saved_persists_across_restart() {
    let db = temp_db();
