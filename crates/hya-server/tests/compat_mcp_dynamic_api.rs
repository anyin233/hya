...
fn tempdir() -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!(
        "hya-server-mcp-dynamic-test-{nanos}-{}",
        std::process::id()
    ));
...
#[tokio::test]
async fn compat_mcp_add_connects_enabled_local_server() {
...
#[tokio::test]
async fn compat_mcp_connect_and_disconnect_update_dynamic_status() {
    let app = router(state().await);
