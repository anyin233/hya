//! Integration tests for `hya-server`: compat experimental api.
...
const WORKDIR: &str = "/tmp/hya-compat-experimental-api";
...
#[tokio::test]
async fn compat_experimental_metadata_routes_return_safe_defaults() {
...
#[tokio::test]
async fn compat_experimental_workspace_tool_session_and_sync_routes_return_safe_defaults() {
    let app = router(state().await);
    ...
    let (status, tools) = request_json(
        app.clone(),
        "GET",
        "/experimental/tool?provider=compat&model=test",
        None,
    )
    .await;
    assert_eq!(status, StatusCode::OK);
    ...
    let (status, _body) = request_json(
        app.clone(),
        "GET",
        "/experimental/tool?provider=compat",
        None,
    )
    .await;
    assert_eq!(status, StatusCode::BAD_REQUEST);
    ...
    assert_eq!(status, StatusCode::OK);
    assert_eq!(history[0]["aggregate_id"].as_str(), Some(session.as_str()));
    ...
    let (status, missing) = request_json(
        ...
        Some(json!({
            ...
            "destination": { "directory": "/tmp/hya-moved" },
            "moveChanges": true
        })),
    )
    .await;
