//! Integration tests for `hya-server`: compat vcs api.
...
#[tokio::test]
async fn compat_vcs_info_falls_back_to_local_default_branch() {
    let workdir = tempdir();
    init_repo_with_head(&workdir);
    git(&workdir, &["branch", "-M", "main"]);
...
#[tokio::test]
async fn compat_vcs_info_uses_configured_default_branch() {
    let workdir = tempdir();
    init_repo_with_head(&workdir);
...
#[tokio::test]
async fn compat_vcs_routes_use_workspace_routing() {
    let default = tempdir();
...
#[tokio::test]
async fn compat_vcs_branch_diff_includes_untracked_files() {
    let workdir = tempdir();
...
#[tokio::test]
async fn compat_vcs_branch_diff_uses_merge_base_for_committed_changes() {
    let workdir = tempdir();
    init_branch_repo(&workdir);
    git(&workdir, &["add", "."]);
...
#[tokio::test]
async fn compat_vcs_branch_diff_preserves_special_filenames() {
    let workdir = tempdir();
    init_repo_with_head(&workdir);
    git(&workdir, &["branch", "-M", "main"]);
    git(&workdir, &["checkout", "-b", "feature"]);
...
#[tokio::test]
async fn compat_vcs_diff_caps_oversized_untracked_patch() {
    let workdir = tempdir();
    init_repo_with_head(&workdir);
...
#[tokio::test]
async fn compat_vcs_diff_caps_total_patch_bytes() {
    let workdir = tempdir();
    init_repo_with_head(&workdir);
