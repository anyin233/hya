//! Integration tests for `hya-server`: compat session legacy message shape api.
...
fn workdir() -> &'static str {
    ...
    WORKDIR.get_or_init(|| {
        let dir = std::env::temp_dir().join("hya-compat-legacy-message-shape-api");
...
#[tokio::test]
async fn legacy_message_info_matches_compat_required_shape() {
...
#[tokio::test]
async fn legacy_session_post_creates_compat_session_info() {
    let app = router(state().await);
...
#[tokio::test]
async fn legacy_session_message_post_accepts_compat_prompt_parts() {
    let app = router(state().await);
    let session = create_session(app.clone()).await;
