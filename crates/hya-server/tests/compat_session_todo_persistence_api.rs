//! Integration tests for `hya-server`: compat session todo persistence api.
...
const WORKDIR: &str = "/tmp/hya-compat-session-todo-persistence-api";
...
#[tokio::test]
async fn compat_session_todo_persists_across_restart() {
    let db = temp_db();
