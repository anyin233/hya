...
const WORKDIR: &str = "/tmp/hya-opencode-workspace-warp-api";
...
#[tokio::test]
async fn opencode_workspace_warp_moves_session_to_worktree_workspace() {
    if !git_available() {
    ...
    let opencode_session = format!("ses_{}", session.replace('-', ""));
    ...
    let loaded = request(app, "GET", &format!("/session/{opencode_session}"), None).await;
