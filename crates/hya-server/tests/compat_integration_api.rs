//! Integration tests for `hya-server`: compat integration api.
...
fn tempdir() -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!(
        "hya-compat-integration-api-{nanos}-{serial}-{}",
        std::process::id()
    ));
...
#[tokio::test]
async fn compat_v2_reference_route_lists_configured_local_references() {
    let workdir = tempdir();
