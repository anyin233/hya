//! Integration tests for `hya-server`: compat session api.
...
fn workdir() -> &'static str {
    ...
    WORKDIR.get_or_init(|| {
        let dir = std::env::temp_dir().join("hya-compat-session-api");
    ...
    let provider = FakeProvider::scripted_turns(vec![
        vec![
            FakeStep::ToolCall {
                ...
                input: json!({
                    "todos": [
                        { "content": "Audit Compat todos", "status": "in_progress", "priority": "high" },
...
#[tokio::test]
async fn compat_session_read_delete_missing_session_returns_not_found() {
...
#[tokio::test]
async fn compat_session_routes_list_get_and_messages() {
    let app = router(state().await);
...
#[tokio::test]
async fn compat_session_update_sets_title_metadata_permission_and_archive() {
    let app = router(state().await);
    let session = create_session(app.clone(), None).await;
    ...
    .await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(metadata_updated["metadata"]["owner"], "compat");
    ...
    assert_eq!(body["metadata"]["owner"], "compat");
    assert_eq!(body["permission"][1]["permission"], "edit");
    assert_eq!(body["time"]["archived"], 42);
}

#[tokio::test]
async fn compat_session_update_missing_session_returns_not_found() {
    let app = router(state().await);
    let missing = SessionId::new().to_string();
...
#[tokio::test]
async fn compat_session_command_and_shell_routes_return_created_messages() {
    let app = router(state().await);
    let session = create_session(app.clone(), None).await;
    ...
    assert!(
        shell["parts"][0]["state"]["output"]
            .as_str()
            .is_some_and(|output| output.contains("compat-shell-ok"))
    );
}

#[tokio::test]
async fn compat_session_command_without_text_uses_skill_template_body() {
    let workdir = isolated_workdir("compat-session-skill-command");
    write_skill(
        &workdir,
        "deploy",
...
#[tokio::test]
async fn compat_session_command_and_init_missing_session_return_not_found() {
    let app = router(state().await);
    let missing = SessionId::new().to_string();
    let expected = json!({
        "name": "NotFoundError",
...
#[tokio::test]
async fn compat_session_shell_busy_returns_legacy_error() {
...
#[tokio::test]
async fn compat_session_command_and_init_busy_return_bad_request() {
    let app = router(shell_state().await);
    let session = create_session(app.clone(), None).await;
    let shell_app = app.clone();
    let shell_session = session.clone();
...
#[tokio::test]
async fn compat_session_shell_missing_session_returns_not_found() {
    let app = router(shell_state().await);
    let missing = SessionId::new().to_string();
    let (status, body) = post_json(
        app,
...
#[tokio::test]
async fn compat_session_delete_missing_session_returns_not_found() {
    let app = router(shell_state().await);
    let missing = SessionId::new().to_string();
    let message = MessageId::new().to_string();
...
#[tokio::test]
async fn compat_session_delete_message_busy_returns_legacy_error() {
    let app = router(shell_state().await);
    let session = create_session(app.clone(), None).await;
    let shell_app = app.clone();
    let shell_session = session.clone();
...
#[tokio::test]
async fn compat_session_delete_removes_session() {
    let app = router(state().await);
    let session = create_session(app.clone(), None).await;
...
#[tokio::test]
async fn compat_session_init_records_requested_command_message() {
    let app = router(state().await);
    let session = create_session(app.clone(), None).await;
    let message = MessageId::new().to_string();
...
#[tokio::test]
async fn compat_session_tree_returns_root_node() {
    let app = router(state().await);
...
#[tokio::test]
async fn compat_session_routes_page_message_and_children() {
    let app = router(state().await);
    let parent = create_session(app.clone(), None).await;
...
#[tokio::test]
async fn compat_session_message_invalid_cursor_returns_bad_request() {
    let app = router(state().await);
    let session = create_session(app.clone(), None).await;
...
#[tokio::test]
async fn compat_session_message_missing_message_returns_not_found() {
    let app = router(state().await);
    let session = create_session(app.clone(), None).await;
...
#[tokio::test]
async fn compat_session_deletes_messages_and_parts() {
    let app = router(state().await);
    let session = create_session(app.clone(), None).await;
    post_prompt(app.clone(), &session).await;
...
#[tokio::test]
async fn compat_session_legacy_deletes_missing_message_and_part_as_noops() {
    let app = router(state().await);
    let session = create_session(app.clone(), None).await;
    post_prompt(app.clone(), &session).await;
...
#[tokio::test]
async fn compat_session_updates_text_parts() {
    let app = router(state().await);
    let session = create_session(app.clone(), None).await;
    post_prompt(app.clone(), &session).await;
...
#[tokio::test]
async fn compat_session_update_part_missing_session_returns_not_found() {
    let app = router(state().await);
    let missing = SessionId::new().to_string();
    let message = MessageId::new().to_string();
    let part = PartId::new().to_string();
...
#[tokio::test]
async fn compat_session_updates_tool_parts_from_compat_state() {
    let app = router(shell_state().await);
    let session = create_session(app.clone(), None).await;
    let (status, shell) = post_json(
        app.clone(),
...
#[tokio::test]
async fn compat_session_diff_returns_recorded_edit_file_diff() {
...
#[tokio::test]
async fn compat_session_diff_filters_recorded_edit_by_part_id() {
...
#[tokio::test]
async fn compat_session_diff_invalid_message_id_returns_bad_request() {
    let app = router(state().await);
    let session = create_session(app.clone(), None).await;
...
#[tokio::test]
async fn compat_session_diff_missing_session_returns_not_found() {
    let app = router(state().await);
    let missing = SessionId::new().to_string();
...
#[tokio::test]
async fn compat_session_share_sets_and_clears_share_url() {
    let app = router(state().await);
    let session = create_session(app.clone(), None).await;
...
#[tokio::test]
async fn compat_session_share_missing_session_returns_not_found() {
    let app = router(state().await);
    let missing = SessionId::new().to_string();
    let expected = json!({
        "name": "NotFoundError",
...
#[tokio::test]
async fn compat_session_fork_missing_session_returns_not_found() {
    let app = router(state().await);
    let missing = SessionId::new().to_string();
    let expected = json!({
        "name": "NotFoundError",
...
#[tokio::test]
async fn compat_session_fork_rejects_invalid_json_body() {
    let app = router(state().await);
    let session = create_session(app.clone(), None).await;
...
#[tokio::test]
async fn compat_session_todo_returns_todowrite_state() {
...
#[tokio::test]
async fn compat_session_todo_returns_empty_array_for_fresh_session() {
    let app = router(state().await);
    let session = create_session(app.clone(), None).await;
