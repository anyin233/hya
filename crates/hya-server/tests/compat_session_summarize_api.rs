//! Integration tests for `hya-server`: compat session summarize api.
...
const WORKDIR: &str = "/tmp/hya-compat-session-summarize-api";
...
#[tokio::test]
async fn compat_session_summarize_persists_summary_message_and_compaction_metadata() {
    ...
    let update = app
        ...
        .oneshot(
            Request::builder()
                ...
                .body(Body::from(
                    json!({"metadata": {"owner": "compat"}}).to_string(),
                ))
                .unwrap(),
        )
        .await
    ...
    assert_eq!(
        body["metadata"]["_hyaCompatCompaction"]["type"],
        "compaction"
    );
    assert_eq!(body["metadata"]["_hyaCompatCompaction"]["auto"], true);
    assert_eq!(
        body["metadata"]["_hyaCompatCompaction"]["providerID"],
        "hya"
    );
    assert_eq!(body["metadata"]["_hyaCompatCompaction"]["modelID"], "fake");
    assert_eq!(
        body["metadata"]["_hyaCompatCompaction"]["messageID"],
        summary["info"]["id"]
    );
    assert_eq!(
        body["metadata"]["_hyaCompatCompaction"]["partID"],
        summary["parts"][0]["id"]
    );
}

#[tokio::test]
async fn compat_session_summarize_missing_session_returns_not_found() {
