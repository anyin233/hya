//! Integration tests for `hya-server`: compat file api.
...
fn tempdir() -> PathBuf {
    ...
    .unwrap();
    std::fs::write(dir.join("README.md"), "# hya\n\nhello docs\n").unwrap();
...
#[tokio::test]
async fn compat_file_routes_return_legacy_shapes() {
...
#[tokio::test]
async fn compat_v2_fs_routes_return_location_wrapped_entries_and_raw_file() {
    let app = router(state(tempdir()).await);
...
#[tokio::test]
async fn compat_v2_fs_routes_honor_location_query_and_headers() {
    let workdir = tempdir();
...
#[tokio::test]
async fn compat_v2_fs_read_rejects_escape_from_scoped_location() {
    let workdir = tempdir();
    let scoped = workdir.join("scoped dir");
    std::fs::create_dir_all(&scoped).unwrap();
    let scoped = std::fs::canonicalize(scoped).unwrap();
