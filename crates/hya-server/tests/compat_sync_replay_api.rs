//! Integration tests for `hya-server`: compat sync replay api.
...
async fn state() -> AppState {
    ...
    AppState::new(
        ...
        Arc::new(AgentSpec {
            ...
            workdir: "/tmp/hya-compat-sync-replay-api".into(),
            reasoning: None,
        }),
    )
}
...
#[tokio::test]
async fn compat_sync_replay_accepts_non_empty_event_history() {
    let (status, body) = post_json(
        router(state().await),
        "/sync/replay",
        json!({
            "directory": "/tmp/hya-compat-sync-replay-api",
            "events": [{
...
#[tokio::test]
async fn compat_sync_replay_events_are_returned_by_history() {
...
#[tokio::test]
async fn compat_sync_history_orders_replayed_events_by_sequence() {
    let app = router(state().await);
...
#[tokio::test]
async fn compat_sync_replay_rejects_mixed_aggregates() {
    let (status, _body) = post_json(
        router(state().await),
        "/sync/replay",
        json!({
...
#[tokio::test]
async fn compat_sync_replay_rejects_non_contiguous_sequences() {
    let (status, _body) = post_json(
        router(state().await),
        "/sync/replay",
        json!({
