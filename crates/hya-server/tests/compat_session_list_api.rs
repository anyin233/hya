//! Integration tests for `hya-server`: compat session list api.
...
const WORKDIR: &str = "/tmp/hya-compat-session-list-api";
...
#[tokio::test]
async fn compat_session_list_hides_empty_unnamed_sessions() {
...
#[tokio::test]
async fn compat_session_list_filters_roots_start_search_and_limit() {
    let app = router(state().await);
