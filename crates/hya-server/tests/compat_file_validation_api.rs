//! Integration tests for `hya-server`: compat file validation api.
...
fn tempdir() -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!(
        "hya-server-file-validation-{nanos}-{serial}-{}",
        std::process::id()
    ));
...
#[tokio::test]
async fn compat_find_file_rejects_invalid_query_params() {
...
#[tokio::test]
async fn compat_find_file_matches_fuzzy_file_queries() {
    let workdir = tempdir();
...
#[tokio::test]
async fn compat_file_list_honors_gitignore_negation() {
    let workdir = tempdir();
...
#[tokio::test]
async fn compat_file_list_honors_anchored_gitignore_patterns() {
    let workdir = tempdir();
...
#[tokio::test]
async fn compat_file_list_matches_unanchored_directory_ignores_at_any_depth() {
    let workdir = tempdir();
...
#[tokio::test]
async fn compat_legacy_file_content_uses_extension_mime_for_binary_files() {
    let workdir = tempdir();
...
#[tokio::test]
async fn compat_file_list_honors_wildcard_ignore_globs() {
    let workdir = tempdir();
...
#[tokio::test]
async fn compat_legacy_file_routes_honor_directory_query() {
    let workdir = tempdir();
