//! Integration tests for `hya-server`: compat session v2 context state api.
...
const WORKDIR: &str = "/tmp/hya-compat-session-v2-context-state-api";
...
#[tokio::test]
async fn compat_v2_context_error_tool_state_includes_result() {
...
#[tokio::test]
async fn compat_v2_context_pending_tool_state_keeps_string_input() {
    let app = router(shell_state().await);
    let (status, created) = post_json(
        app.clone(),
        "/api/session".to_string(),
