//! Integration tests for `hya-server`: compat interaction root api.
...
fn tempdir() -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!(
        "hya-server-interaction-root-test-{nanos}-{}",
        std::process::id()
    ));
...
#[tokio::test]
async fn compat_permission_root_routes_list_reply_and_type_missing_errors() {
...
#[tokio::test]
async fn compat_question_root_routes_list_reply_reject_and_type_missing_errors() {
