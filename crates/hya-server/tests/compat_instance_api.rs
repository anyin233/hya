//! Integration tests for `hya-server`: compat instance api.
...
fn tempdir() -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!(
        "hya-server-instance-test-{nanos}-{}",
        std::process::id()
    ));
    std::fs::create_dir_all(dir.join(".hya/skills/demo")).unwrap();
    std::fs::write(
        dir.join(".hya/skills/demo/SKILL.md"),
...
#[tokio::test]
async fn compat_instance_routes_return_metadata() {
    let workdir = tempdir();
...
#[tokio::test]
async fn compat_agent_routes_expose_permission_rules() {
    let app = router(
        state_with_rules(
            tempdir(),
...
#[tokio::test]
async fn compat_instance_metadata_routes_use_workspace_routing() {
    let workdir = tempdir();
    let scoped = workdir.join("scoped dir");
    std::fs::create_dir_all(scoped.join(".hya/skills/scoped")).unwrap();
    std::fs::write(
        scoped.join(".hya/skills/scoped/SKILL.md"),
...
#[tokio::test]
async fn compat_vcs_routes_return_status_diff_and_apply_patch() {
    let workdir = tempdir();
