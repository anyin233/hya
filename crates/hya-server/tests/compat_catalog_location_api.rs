//! Integration tests for `hya-server`: compat catalog location api.
...
fn tempdir() -> std::path::PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!(
        "hya-server-catalog-location-test-{nanos}-{}",
        std::process::id()
    ));
...
#[tokio::test]
async fn compat_catalog_routes_honor_location_query_and_headers() {
    let workdir = tempdir();
