//! Integration tests for `hya-server`: compat fs v2 api.
...
fn tempdir() -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!(
        "hya-server-fs-v2-test-{nanos}-{}-{seq}",
        std::process::id()
    ));
...
#[tokio::test]
async fn compat_v2_fs_list_orders_directories_first_with_trailing_slash() {
    let workdir = tempdir();
...
#[tokio::test]
async fn compat_v2_fs_list_uses_extension_mime_types() {
    let workdir = tempdir();
...
#[tokio::test]
async fn compat_v2_fs_list_matches_js_mime_types_database_aliases() {
    let workdir = tempdir();
    for path in [
        "app.rs",
        "config.toml",
...
#[tokio::test]
async fn compat_v2_fs_find_uses_compat_default_limit() {
    let workdir = tempdir();
    for index in 0..12 {
...
#[tokio::test]
async fn compat_v2_fs_find_accepts_unbounded_positive_limits() {
    let workdir = tempdir();
    for index in 0..205 {
...
#[tokio::test]
async fn compat_v2_fs_find_matches_fuzzy_file_queries() {
    let workdir = tempdir();
...
#[tokio::test]
async fn compat_v2_fs_find_preserves_directory_trailing_slash() {
    let workdir = tempdir();
