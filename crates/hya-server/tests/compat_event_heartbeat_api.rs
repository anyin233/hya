//! Integration tests for `hya-server`: compat event heartbeat api.
...
const WORKDIR: &str = "/tmp/hya-compat-event-heartbeat-api";
...
#[tokio::test]
async fn compat_event_routes_stream_heartbeat_events() {
    assert_event_heartbeat("/event").await;
    assert_event_heartbeat(
        "/api/event?location%5Bdirectory%5D=/tmp/hya-compat-event-heartbeat-api",
    )
    .await;
}

#[tokio::test]
async fn compat_global_event_route_streams_heartbeat_events() {
