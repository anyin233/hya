//! Integration tests for `hya-server`: compat pty api.
...
fn tempdir() -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!(
        "hya-compat-pty-api-{nanos}-{serial}-{}",
        std::process::id()
    ));
...
#[tokio::test]
async fn compat_pty_routes_report_shells_and_manage_session_metadata() {
    let workdir = tempdir();
...
#[tokio::test]
async fn compat_v2_pty_routes_wrap_location_and_manage_session_metadata() {
    let workdir = tempdir();
    let workdir_text = workdir.to_string_lossy().into_owned();
    let app = router(state(&workdir).await);
...
#[tokio::test]
async fn compat_pty_connect_token_rejects_cross_origin_requests() {
    let workdir = tempdir();
    let app = router(state(&workdir).await);
    let (status, created) = request(
        app.clone(),
...
#[tokio::test]
async fn compat_pty_connect_reports_missing_session_before_ticket_validation() {
    let workdir = tempdir();
    let app = router(state(&workdir).await);
