...
async fn interactions_only_stream_carries_root_session_list_frames() {
    ...
    // Project-list notices (`projectsUpdated`, ADR-0024) share this stream
    // (the root session joined the Project of its workdir); they are not
    // session-list frames.
    let frames = frames.await.unwrap();
    assert!(frames.iter().any(is_projects_updated), "{frames:#?}");
    let frames: Vec<Value> = frames
        .into_iter()
        .filter(|frame| !is_projects_updated(frame))
        .collect();
...
async fn session_list_frames_match_over_grpc() {
    ...
    tokio::time::timeout(Duration::from_secs(10), async {
        while let Some(frame) = events.next().await {
            ...
            let kind = match &event.payload {
                ...
                // Project-list notices (ADR-0024) are not session-list frames.
                Some(pb::stream_event::Payload::ProjectsUpdated(_)) => continue,
                _ => "other".to_owned(),
            };
    ...
    let sse_rows: Vec<(String, String)> = sse
        .await
        .unwrap()
        .iter()
        .filter(|frame| !is_projects_updated(frame))
