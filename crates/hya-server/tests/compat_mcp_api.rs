//! Integration tests for `hya-server`: compat mcp api.
...
#[tokio::test]
async fn compat_mcp_status_reports_configured_servers() {
...
#[tokio::test]
async fn compat_mcp_add_accepts_disabled_local_server() {
    let mut configs = BTreeMap::new();
    configs.insert(
...
#[tokio::test]
async fn compat_mcp_add_rejects_invalid_config() {
...
#[tokio::test]
async fn compat_mcp_connect_disconnect_acknowledge_known_server() {
    let app = router(state().await);
    let added = request(
        app.clone(),
        Method::POST,
...
#[tokio::test]
async fn compat_mcp_auth_routes_return_deterministic_responses_for_known_non_oauth_server() {
    let app = router(state().await);
    let added = request(
        app.clone(),
        Method::POST,
...
#[tokio::test]
async fn compat_mcp_routes_return_typed_not_found_for_missing_server() {
    let app = router(state().await);
