//! Integration tests for `hya-server`: compat session switch api.
...
const WORKDIR: &str = "/tmp/hya-compat-session-switch-api";
...
#[tokio::test]
async fn compat_v2_session_switch_routes_update_selected_agent_and_model() {
...
#[tokio::test]
async fn compat_v2_session_context_records_agent_and_model_switch_messages() {
    let app = router(state().await);
    let session = create_session(app.clone()).await;
