//! Integration tests for `hya-server`: compat tui api.
...
const WORKDIR: &str = "/tmp/hya-compat-tui-api";
...
#[tokio::test]
async fn compat_tui_publish_queues_control_requests() {
...
#[tokio::test]
async fn compat_tui_direct_routes_queue_control_requests() {
    let app = router(state().await);
    let cases = [
        (
            "/tui/append-prompt",
...
#[tokio::test]
async fn compat_tui_select_session_validates_and_queues_existing_sessions() {
    let app = router(state().await);
    ...
    let missing = hya_proto::SessionId::new().to_string();
    let (status, _) = request(
        app.clone(),
        "POST",
        "/tui/select-session",
