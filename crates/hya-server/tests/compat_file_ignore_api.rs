//! Integration tests for `hya-server`: compat file ignore api.
...
fn tempdir() -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!(
        "hya-server-file-ignore-{nanos}-{serial}-{}",
        std::process::id()
    ));
...
#[tokio::test]
async fn compat_file_list_matches_slash_aware_ignore_globs() {
    let workdir = tempdir();
    std::fs::write(
...
#[tokio::test]
async fn compat_file_list_honors_escaped_ignore_literals() {
    let workdir = tempdir();
    std::fs::write(
        workdir.join(".gitignore"),
        "literal\\*.log\nliteral\\[ab].log\n\\#hash.log\n\\!important.log\nspace\\ name.log\n",
...
#[tokio::test]
async fn compat_file_list_preserves_significant_ignore_rule_spaces() {
    let workdir = tempdir();
    std::fs::write(
        workdir.join(".gitignore"),
        " leading.log\nescaped-trailing\\ \n*.keep\n! keep.keep\nescaped-keep*\n!escaped-keep\\ \n",
