...
async fn read_reference_state(workdir: &str, reference_dir: &str) -> AppState {
    ...
    let reference_dir = std::fs::canonicalize(reference_dir).unwrap();
    let reference_file = reference_dir.join("guide.txt");
    std::fs::write(&reference_file, "reference body\n").unwrap();
    let provider = FakeProvider::scripted_turns(vec![
        vec![
            FakeStep::ToolCall {
                name: "read".to_string(),
                input: json!({ "filePath": reference_file }),
            },
