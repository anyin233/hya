//! Integration tests for `hya-server`: compat provider model api.
...
fn workdir() -> &'static str {
    ...
    WORKDIR.get_or_init(|| {
        let dir = std::env::temp_dir().join("hya-compat-provider-model-api");
...
fn tempdir() -> std::path::PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!(
        "hya-server-metadata-test-{nanos}-{}",
        std::process::id()
    ));
...
#[tokio::test]
async fn compat_v2_metadata_routes_return_location_wrapped_data() {
    let workdir = tempdir();
