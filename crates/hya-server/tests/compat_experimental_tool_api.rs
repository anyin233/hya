//! Integration tests for `hya-server`: compat experimental tool api.
...
const WORKDIR: &str = "/tmp/hya-compat-experimental-tool-api";
...
async fn compat_experimental_tool_list_exposes_websearch_for_every_provider() {
    ...
    let (status, compat_body) =
        get_json(app.clone(), "/experimental/tool?provider=compat&model=test").await;
    assert_eq!(status, StatusCode::OK);
    let compat_ids = tool_ids(&compat_body);
    assert!(compat_ids.contains(&"websearch"));
