...
fn expected_workdir(workdir: &str) -> String {
    ...
    let mut path = std::env::var_os("XDG_DATA_HOME").map_or_else(
        || {
            std::env::var_os("HOME")
                ...
                .join(".local/share/opencode/repos")
        },
        |data| PathBuf::from(data).join("opencode").join("repos"),
    );
...
#[tokio::test]
async fn opencode_reference_api_lists_git_sources() {
    let workdir = workdir();
