//! Integration tests for `hya-server`: compat event api.
...
const WORKDIR: &str = "/tmp/hya-compat-event-api";
...
#[tokio::test]
async fn compat_v2_event_route_streams_connected_event() {
    let app = router(state().await);
    let resp = app
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/api/event?location%5Bdirectory%5D=/tmp/hya-compat-event-api")
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
#[tokio::test]
async fn compat_legacy_event_route_streams_connected_event() {
    assert_event_stream("/event").await;
}

#[tokio::test]
async fn compat_global_event_route_streams_connected_event() {
    let app = router(state().await);
    let resp = app
        .oneshot(
            Request::builder()
...
#[tokio::test]
async fn compat_legacy_event_route_streams_step_parts() {
...
#[tokio::test]
async fn compat_v2_event_route_streams_next_step_events() {
    let app = router(text_state().await);
    let resp = app
        .clone()
        .oneshot(
    ...
    assert_eq!(started["data"]["model"]["providerID"], "hya");
...
#[tokio::test]
async fn compat_v2_event_route_streams_session_created_location() {
    let app = router(state().await);
    let directory = "/tmp/hya-compat-event-api-scoped";
    let resp = app
        .clone()
        .oneshot(
            Request::builder()
...
#[tokio::test]
async fn compat_v2_event_route_filters_session_events_by_location() {
    let app = router(state().await);
    let visible = "/tmp/hya-compat-event-api-visible";
    let hidden = "/tmp/hya-compat-event-api-hidden";
    let resp = app
        .clone()
        .oneshot(
            Request::builder()
...
#[tokio::test]
async fn compat_legacy_event_route_streams_session_updated_properties() {
    let app = router(state().await);
    let resp = app
        .clone()
        .oneshot(
...
#[tokio::test]
async fn compat_legacy_event_route_streams_message_updated_properties() {
...
#[tokio::test]
async fn compat_legacy_event_route_streams_reasoning_part_events() {
...
#[tokio::test]
async fn compat_legacy_event_route_streams_tool_part_events() {
    ...
    let shell = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header("content-type", "application/json")
                .body(Body::from(
                    json!({"command": "printf compat-tool-event"}).to_string(),
                ))
                .unwrap(),
        )
        .await
    ...
    assert!(
        completed["properties"]["part"]["state"]["output"]
            .as_str()
            .is_some_and(|output| output.contains("compat-tool-event"))
    );
