//! Integration tests for `hya-server`: compat session v2 message api.
...
const WORKDIR: &str = "/tmp/hya-compat-session-v2-message-api";
...
#[tokio::test]
async fn compat_v2_session_message_route_paginates_projected_messages() {
...
#[tokio::test]
async fn compat_v2_session_message_route_supports_before_headers() {
    let app = router(state().await);
    let (status, created) = post_json(
        app.clone(),
        "/api/session".to_string(),
...
#[tokio::test]
async fn compat_v2_session_message_limit_zero_returns_all_messages() {
    let app = router(state().await);
    let (status, created) = post_json(
        app.clone(),
        "/api/session".to_string(),
