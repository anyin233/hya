//! Integration tests for `hya-server`: compat command metadata api.
...
#[tokio::test]
async fn compat_command_route_keeps_project_hya_before_home_duplicate() {
    let workdir = tempdir();
    let home = tempdir();
...
async fn compat_command_route_includes_native_init_review_and_workflow_commands() {
    ...
    // When: the Compat /command route is listed.
    let (status, commands) = get_json(app, "/command").await;

    // Then: Compat's native init and review commands are advertised, but the
...
#[tokio::test]
async fn compat_command_route_exposes_workspace_skills_as_commands() {
    ...
    // When: the Compat /command route is listed for that workspace.
    let (status, commands) =
        get_json(app, &format!("/command?directory={}", workdir.display())).await;
...
#[tokio::test]
async fn compat_command_route_discovers_project_markdown_commands() {
    // Given: a workspace with Compat command markdown files on disk.
    let workdir = tempdir();
    ...
    // Then: file commands are exposed with Compat-compatible names and metadata.
    assert_eq!(status, StatusCode::OK);
    let review = find_command(&commands, "review");
...
#[tokio::test]
async fn compat_command_route_discovers_inline_config_commands() {
    // Given: a workspace with inline Compat command config files.
    let workdir = tempdir();
    std::fs::write(
  ...
  "command": {
    ...
    std::fs::write(
        ...
        r#"{
  // Newer Compat config also accepts "commands".
  "commands": {
    "review": {
