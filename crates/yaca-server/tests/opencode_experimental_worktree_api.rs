...
#[tokio::test]
async fn opencode_experimental_worktree_create_list_and_remove_use_git_worktrees() {
    if !git_available() {
...
#[tokio::test]
async fn opencode_experimental_worktree_reset_restores_and_cleans_directory() {
    if !git_available() {
        eprintln!("skipping: git is not available");
        return;
    }
...
#[tokio::test]
async fn opencode_experimental_worktree_create_rejects_invalid_payload() {
