...
fn tempdir() -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!(
        "yaca-server-permission-question-test-{nanos}-{}",
        std::process::id()
    ));
...
#[tokio::test]
async fn opencode_permission_request_lists_and_replies_by_session() {
...
#[tokio::test]
async fn opencode_permission_always_resolves_matching_session_requests() {
    let (permission, permission_rx) = PermissionPlane::new(PermissionRules::default());
