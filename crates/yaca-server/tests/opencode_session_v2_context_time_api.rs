...
use yaca_proto::{AgentName, Event, MessageId, ModelRef, Role, SessionId};
use yaca_provider::{
...
const WORKDIR: &str = "/tmp/yaca-opencode-session-v2-context-time-api";
...
async fn pending_state() -> yaca_server::AppState {
...
async fn opencode_v2_context_omits_unfinished_assistant_completed_time() {
    ...
    let app = yaca_server::router(state);
...
async fn await_assistant_started(
    events: &mut tokio::sync::broadcast::Receiver<yaca_proto::Envelope>,
) {
