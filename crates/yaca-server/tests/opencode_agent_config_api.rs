...
fn tempdir() -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!(
        "yaca-server-agent-config-test-{nanos}-{}",
        std::process::id()
    ));
...
#[tokio::test]
async fn opencode_agent_routes_discover_inline_config_agents() {
    // Given: OpenCode config files with inline agents and modes.
    let workdir = tempdir();
