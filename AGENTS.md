...
| `crates/hya-app` | Runtime composition library (not a binary). Config load, provider/auth resolution, MCP and plugin wiring, permission policy construction, session engine build, installed-bundle catalog refresh. Prefer this crate over `hya-backend` when changing composition, not CLI surface. |
| `crates/hya-bundle` | AgentBundle prepare/validate/catalog types and package fixtures. Catalog builders and resource/agent resolution used by install CLI and process E2E. Prefer this crate for bundle authoring contracts and prepare semantics. |
...
| `crates/xtask` | Dev-tooling entry point with working tasks: `sync-compat`, `migrate`, `startup-bench`, and `matrix-check`. |
