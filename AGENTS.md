...
| `crates/hya-relay` | Secure relay building blocks: the `hya.relay.v1` rendezvous protocol (`proto/hya/relay/v1`, separate from `hya.v1`; generated prost/tonic code committed, regenerate with `cargo run -p xtask -- gen-relay`), the binding-independent `RelayTransport` message-stream abstraction with an in-memory pair for tests. Must not depend on hya runtime crates. See `docs/relay.md`. |
