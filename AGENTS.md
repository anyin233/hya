...
| `crates/hya-relay` | Secure relay building blocks: the `hya.relay.v1` rendezvous protocol (`proto/hya/relay/v1`, separate from `hya.v1`; generated prost/tonic code committed, regenerate with `cargo run -p xtask -- gen-relay`), the binding-independent `RelayTransport` message-stream abstraction with an in-memory pair for tests, and the `RelayLink` (`hya://…` link grammar, room-id derivation). Must not depend on hya runtime crates. See `docs/relay.md`. |
...
| `packages/hya-tui` | Bun/OpenTUI TUI over the v1 HTTP/JSON+SSE contract: sessions, transcript, turns, pending interactions, models, Workflows, the Provider View (`/key`: providers, keys, model lists, model tests), slash completion, and a generic `/api` command view. Started by bare `hya` (`--server <in-process URL> --web-url|--web-error`); shipped as `lib/hya/tui` in the release archive and source install, so it must stay self-contained (no imports outside the package; the `/api` catalog `src/operations.json` is generated by `gen-api`). Also runs directly with Bun for development (`bun packages/hya-tui/src/main.ts`). See `docs/tui.md`. |
