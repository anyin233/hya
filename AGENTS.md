...
| `crates/hya-tool` | Tool and permission plane. Provides the `Tool` trait, the 27-name canonical registry plus hidden aliases, allow/ask/deny rules, interaction/question requests, spawn/todo/skill/websearch/LSP/Workflow planes, and builtins for read/write/edit/apply-patch, Bash, web fetch/search, task/announce, Workflow, todo, and invalid-tool handling. |
