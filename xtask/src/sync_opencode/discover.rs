...
use crate::sync_opencode::cli::Args;
...
pub(crate) struct OpencodeConfig {
    ...
    #[serde(default)]
    mcp: BTreeMap<String, OpencodeMcp>,
}

#[derive(Debug, Default, Deserialize)]
struct OpencodeSkills {
    #[serde(default)]
    paths: Vec<PathBuf>,
}
...
pub(crate) fn load_opencode_config(path: &Path) -> Result<OpencodeConfig> {
    let raw = std::fs::read_to_string(path)
        .with_context(|| format!("read OpenCode config {}", path.display()))?;
    serde_json::from_str(&raw).with_context(|| format!("parse OpenCode config {}", path.display()))
}

pub(crate) fn collect_skill_roots(args: &Args, config: &OpencodeConfig) -> Vec<PathBuf> {
    let mut roots = args.opencode_skill_roots.clone();
...
pub(crate) fn collect_supported_mcp(config: &OpencodeConfig) -> Vec<McpCandidate> {
    config
        .mcp
        .iter()
