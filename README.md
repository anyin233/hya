...
The shipped binary is `hya` (crate [`crates/hya-cli`](crates/hya-cli), see
its `[[bin]]` entry). Run it directly from the workspace:

```sh
cargo run -p hya-cli --            # interactive TUI
...
- Headless single-turn execution (`hya exec` / `hya run`) and iterative goal
  mode (`hya -p "<goal>"`).
- HTTP/SSE server (`hya serve`) exposing native `/sessions/*` routes plus
