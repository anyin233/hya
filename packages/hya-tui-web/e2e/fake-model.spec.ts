// Verifies the fake OpenAI-compatible model (fake-model.ts) and the `model`
// backend fixture option through the v1 HTTP API directly, with no TUI
// rendering involved: a scripted text reply becomes the assistant message, a
// scripted tool call round-trips through a real builtin tool and a follow-up
// text step finishes the turn, and the fake server records what it received.

import { expect, textStep, test, toolStep, type Backend } from "./hya"
...
type MessageInfo = { id: string; role: string; parts?: MessagePart[] }
