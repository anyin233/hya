...
test.describe("hya TUI Saved Rules view", () => {
  ...
  test("a bash always-allow answer is a saved rule; /rules lists and deletes it", async ({ tui, backend }) => {
    ...
    await term.waitForText(/EFFECT\s+TOOL\s+PATTERN\s+PROJECT\s+·\s+SAVED/)
    // Readable row: effect, tool, the exact bash command as the pattern, the
    // "global" project (a bash grant is never Project-scoped), and a relative
    // "saved" time.
    await term.waitForText(/allow\s+bash\s+echo saved-rule\s+global\s+·\s+\d+s ago/)
