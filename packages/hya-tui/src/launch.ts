...
* most recently updated top-level session of `projectId` that is not
 * archived — the Project ensured for `--dir` (state/projects.ts
 * `newestTopLevelSession`; list order breaks ties) — else none. Without a
 * Project (`--remote`) there is nothing to continue.
 */
