...
test("resumeRows: the active Project's root sessions incl. archived, newest first, archived marked", () => {
  const rows = resumeRows(sessions, "prj_w", "old", now)
  ...
  // Without an active Project (a `--remote` start): every root session.
  expect(resumeRows(sessions, undefined, undefined, now).map((row) => row.id)).toEqual(["elsewhere", "new", "arch", "old"])
})
...
test("sessionRows marks archived sessions; --continue never picks an archived one", () => {
  ...
  expect(initialSessionId(sessions, { continue: true }, "prj_w")).toBe("new")
})
...
test("the resumer unarchives before it opens; without an id it offers the active Project's sessions incl. archived", async () => {
  const store = createAppStore()
  ...
  const resumer = createResumer({
    store,
    client: {
      listSessions: async (options) => { listed.push(options); return sessions },
  ...
  expect(listed.at(-1)).toEqual({ includeArchived: true, projectId: "prj_w" })
