...
export function createStaticPluginHost(): TuiPluginHost {
  ...
  return {
    async start(input) {
        ...
        plugins.map(async (plugin, index) => {
          ...
          const lifecycle: TuiPluginApi["lifecycle"] = {
            ...
            onDispose(fn) {
              track(fn)
              return () => {
                const cleanupIndex = cleanups.indexOf(fn)
          ...
          const pluginSlots: TuiPluginApi["slots"] = {
            register<Slots extends Record<string, object>>(slot: TuiSlotPlugin<Slots>) {
              const id = `${plugin.id}:${index}:${cleanups.length}`
      ...
      // Stable status order matching builtin declaration order.
      const order = new Map(plugins.map((plugin, index) => [plugin.id, index]))
