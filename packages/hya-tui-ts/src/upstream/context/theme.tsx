...
import { HyaPaths } from "../../hya/platform"
...
const themeSource: ThemeSource = {
  async discover() {
    const directories = [HyaPaths.config]
    for (let current = process.cwd(); ; current = path.dirname(current)) {
      directories.push(path.join(current, ".hya"))
...
export async function discoverThemes(directories: string[]) {
  ...
  for (const directory of directories) {
    ...
    const files = await readdir(themeDirectory).catch(() => [])
    for (const name of files) {
      if (!name.endsWith(".json")) continue
      const file = path.join(themeDirectory, name)
  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "hya")
        draft.active = typeof active === "string" ? active : "hya"
        draft.ready = false
      }),
    )
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "hya")
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.hya, store.mode)
    })
