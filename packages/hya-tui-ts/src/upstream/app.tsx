...
import { STATUS_COMMAND, terminalTitle } from "../hya/product"
...
export const run = Effect.fn("Tui.run")(function* (input: TuiInput) {
  const global = yield* HyaPlatform
      ...
      const renderer = yield* Effect.acquireRelease(
        Effect.tryPromise(() =>
          createCliRenderer({
            ...
            useMouse: !HyaFlag.disableMouse && input.config.mouse,
            consoleOptions: {
                ...
                <ErrorBoundary fallback={(error, reset) => <ErrorComponent error={error} reset={reset} mode={mode} />}>
                  ...
                  >
                    ...
                    >
                      <TuiStartupProvider
                        value={{
                          initialRoute: process.env.HYA_ROUTE ? JSON.parse(process.env.HYA_ROUTE) : undefined,
                          skipInitialLoading: Boolean(process.env.HYA_FAST_BOOT),
                        }}
                      >
                        <ClipboardProvider>
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!HyaFlag.disableCopyOnSelect) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || HyaFlag.disableTerminalTitle) return
    ...
    if (route.data.type === "session") {
      ...
      renderer.setTerminalTitle(terminalTitle(title))
      return
    }

    if (route.data.type === "plugin") {
      renderer.setTerminalTitle(terminalTitle(route.data.id))
    }
  })
  ...
  return (
    <box
      ...
      onMouseUp={
        !HyaFlag.disableCopyOnSelect
          ? () => Selection.copy(renderer, toast, clipboard)
          : undefined
      }
    >
      <Show when={HyaFlag.showTimeToFirstDraw}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
